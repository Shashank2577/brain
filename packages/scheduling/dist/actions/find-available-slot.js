...
export default defineAction({
    description: "Find the next available time slot for an event type, optionally near a preferred time",
    schema: z.object({
    ...
    run: async (args) => {
        ...
        if (args.preferredTime) {
            ...
            const nearest = slots.reduce((best, s) => Math.abs(new Date(s.start).getTime() - target) <
