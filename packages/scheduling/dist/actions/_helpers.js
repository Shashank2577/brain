...
export async function assertTeamAdmin(teamId) {
    const { getDb, schema } = getSchedulingContext();
    const email = currentUserEmail();
    const rows = await getDb()
        .select({ role: schema.teamMembers.role })
        .from(schema.teamMembers)
        .where(and(eq(schema.teamMembers.teamId, teamId), eq(schema.teamMembers.userEmail, email)))
        .limit(1);
 ...
 */
export async function assertTeamMember(teamId) {
    const { getDb, schema } = getSchedulingContext();
    const email = currentUserEmail();
    const rows = await getDb()
        .select({ role: schema.teamMembers.role })
