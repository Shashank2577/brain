...
function getStateSecret() {
    const secret = process.env.SCHEDULING_OAUTH_STATE_SECRET ||
        process.env.BETTER_AUTH_SECRET ||
        process.env.AUTH_SECRET;
    if (!secret) {
        throw new Error("OAuth state secret is not configured. Set BETTER_AUTH_SECRET (or SCHEDULING_OAUTH_STATE_SECRET).");
    }
    return secret;
}
function b64url(buf) {
...
export function verifyVideoOAuthState(opts) {
    ...
    if (!nonce || !timestamp || !kindFragment || !providedSig)
        return false;
    const expectedKind = safeFragment(opts.kind);
    if (kindFragment !== expectedKind)
        return false;
    const safeEmail = safeFragment(opts.userEmail.toLowerCase());
...
//# sourceMappingURL=video-oauth-state.js.map