...
export async function getAvailableSlots(input) {
    ...
    const counts = await bookingCountsFor(input.forUserEmail, input.eventType.id, input.viewerTimezone ?? schedule.timezone);
...
async function resolveDefaultSchedule(ownerEmail) {
    const { getDb, schema } = getSchedulingContext();
    const rows = await getDb()
        .select()
        .from(schema.schedules)
        .where(and(eq(schema.schedules.ownerEmail, ownerEmail), eq(schema.schedules.isDefault, true)));
    if (!rows[0])
        return null;
...
export async function aggregateBusy(input) {
    ...
    const bookings = await db
        ...
        .where(and(eq(schema.bookings.hostEmail, input.userEmail), eq(schema.bookings.status, "confirmed"), gte(schema.bookings.endTime, input.rangeStart.toISOString()), lt(schema.bookings.startTime, input.rangeEnd.toISOString())));
    ...
    const creds = await db
        ...
        .where(and(eq(schema.schedulingCredentials.userEmail, input.userEmail), eq(schema.schedulingCredentials.invalid, false)));
...
async function bookingCountsFor(userEmail, eventTypeId, timezone) {
    const { getDb, schema } = getSchedulingContext();
    const rows = await getDb()
        .select()
        .from(schema.bookings)
        .where(and(eq(schema.bookings.hostEmail, userEmail), eq(schema.bookings.eventTypeId, eventTypeId), eq(schema.bookings.status, "confirmed")));
    const counts = {
        perDay: {},
        perWeek: {},
        perMonth: {},
...
//# sourceMappingURL=availability-engine.js.map