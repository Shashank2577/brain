...
export async function listSchedules(ownerEmailOrParams) {
    const params = typeof ownerEmailOrParams === "string"
    ...
    return scheduleRows.map((s) => hydrateSchedule(s, availabilityRows, overrideRows));
}
...
export async function updateSchedule(id, patch) {
    ...
    const db = getDb();
    const now = new Date().toISOString();
    const setClause = { updatedAt: now };
    if (patch.name != null)
        setClause.name = patch.name;
    if (patch.timezone != null)
        setClause.timezone = patch.timezone;
    if (patch.isDefault != null)
        setClause.isDefault = patch.isDefault;
    await db
...
export async function setDefaultSchedule(ownerEmail, scheduleId) {
    const { getDb, schema } = getSchedulingContext();
    const db = getDb();
    const now = new Date().toISOString();
    const all = await db
...
export async function upsertDateOverride(scheduleId, date, intervals) {
    const { getDb, schema } = getSchedulingContext();
    const db = getDb();
    const now = new Date().toISOString();
    await db
        .delete(schema.dateOverrides)
        .where(and(eq(schema.dateOverrides.scheduleId, scheduleId), eq(schema.dateOverrides.date, date)));
    await db.insert(schema.dateOverrides).values({
        id: nanoid(),
        scheduleId,
        date,
...
export async function removeDateOverride(scheduleId, date) {
    const { getDb, schema } = getSchedulingContext();
    await getDb()
        .delete(schema.dateOverrides)
        .where(and(eq(schema.dateOverrides.scheduleId, scheduleId), eq(schema.dateOverrides.date, date)));
}
function hydrateSchedule(s, availabilityRows, overrideRows) {
    const weekly = new Map();
    for (const row of availabilityRows) {
        if (row.scheduleId !== s.id)
            continue;
        const day = row.day;
        if (!weekly.has(day))
            weekly.set(day, []);
    ...
    const weeklyAvailability = Array.from(weekly.entries())
        .map(([day, intervals]) => ({ day, intervals }))
        .sort((a, b) => a.day - b.day);
    const dateOverrides = overrideRows
...
//# sourceMappingURL=schedules-repo.js.map