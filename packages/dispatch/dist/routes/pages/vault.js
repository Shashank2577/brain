...
function VaultAccessSettingsCard({ mode }) {
    const update = useActionMutation("set-vault-access-settings", {
        onSuccess: (next) => toast.success(next?.mode === "manual"
            ? "Manual vault access enabled"
            : "All apps can use vault keys"),
        onError: (err) => toast.error(String(err)),
    });
    const allApps = mode !== "manual";
    return (_jsx("div", { className: "rounded-xl border bg-card px-4 py-3", children: _jsxs("div", { className: "flex items-center justify-between gap-4", children: [_jsxs("div", { className: "min-w-0", children: [_jsx(Label, { className: "text-sm font-medium", children: "All apps can use vault keys" }), _jsx("p", { className: "mt-1 text-xs text-muted-foreground", children: allApps
                                ? "Every workspace app can receive every saved key."
                                : "Only apps with explicit grants can receive saved keys." })] }), _jsx(Switch, { checked: allApps, disabled: update.isPending, onCheckedChange: (checked) => update.mutate({ mode: checked ? "all-apps" : "manual" }), "aria-label": "Allow all workspace apps to use vault keys" })] }) }));
}
function SecretRow({ secret, grants, accessMode, }) {
