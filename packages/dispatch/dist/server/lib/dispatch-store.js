...
const _linkFailureMap = new Map();
function linkFailureKey(platform, externalUserId) {
    return `${platform}::${externalUserId ?? ""}`;
}
function getLinkFailureState(key) {
...
function isLinkAttemptBlocked(platform, externalUserId) {
    if (!externalUserId)
...
function recordLinkFailure(platform, externalUserId) {
    if (!externalUserId)
        return;
    const key = linkFailureKey(platform, externalUserId);
    ...
    state.failures = state.failures.filter((ts) => nowMs - ts < LINK_FAIL_WINDOW_MS);
...
function clearLinkFailures(platform, externalUserId) {
    if (!externalUserId)
        return;
...
export class LinkRateLimitError extends Error {
    ...
    constructor(retryAfterMs) {
        super(`Too many failed link attempts. Try again in ${Math.ceil(retryAfterMs / 60_000)} minute(s).`);
        this.retryAfterMs = retryAfterMs;
    }
}
export function currentOwnerEmail() {
...
export function requireDispatchCtx() {
...
export async function listAuditEvents(limit = 50) {
    ...
    return db
        ...
        .where(and(eq(schema.dispatchAuditEvents.ownerEmail, currentOwnerEmail()), orgId
...
export async function listDestinations() {
    ...
    return db
        ...
        .where(and(eq(schema.dispatchDestinations.ownerEmail, currentOwnerEmail()), orgId
...
export async function getDestinationById(destinationId, ctx = requireDispatchCtx()) {
    const db = getDb();
    const [row] = await db
        .select()
        .from(schema.dispatchDestinations)
        .where(and(eq(schema.dispatchDestinations.id, destinationId), ctxScope(schema.dispatchDestinations, ctx)))
        .limit(1);
    return row ?? null;
}
async function applyDestinationUpsert(input, actor = currentOwnerEmail(), ctx = requireDispatchCtx()) {
    const db = getDb();
    const timestamp = now();
...
async function applyDestinationDelete(destinationId, actor = currentOwnerEmail(), ctx = requireDispatchCtx()) {
    const db = getDb();
...
async function notifyApprovers(requestId, summary) {
    const policy = await getApprovalPolicy();
    const apiKey = process.env.SENDGRID_API_KEY;
    const from = process.env.SENDGRID_FROM_EMAIL;
    const appUrl = process.env.APP_URL;
    if (!apiKey || !from || !appUrl || policy.approverEmails.length === 0)
        return;
...
export async function createApprovalRequest(input) {
    ...
    await db.insert(schema.dispatchApprovalRequests).values({
        ...
        beforeValue: input.beforeValue === undefined ? null : safeJson(input.beforeValue),
        afterValue: input.afterValue === undefined ? null : safeJson(input.afterValue),
...
export async function listApprovalRequests() {
    ...
    return db
        ...
        .where(and(eq(schema.dispatchApprovalRequests.ownerEmail, currentOwnerEmail()), orgId
...
async function getApprovalRequest(requestId, ctx = requireDispatchCtx()) {
    const db = getDb();
    const [row] = await db
        .select()
        .from(schema.dispatchApprovalRequests)
        .where(and(eq(schema.dispatchApprovalRequests.id, requestId), ctxScope(schema.dispatchApprovalRequests, ctx)))
        .limit(1);
    return row ?? null;
}
async function applyApprovedRequest(request) {
    ...
    if (request.changeType === "destination.upsert") {
        return applyDestinationUpsert(payload, request.reviewedBy || currentOwnerEmail(), requestCtx);
    }
    ...
    if (request.changeType === "dream-proposal.apply") {
        ...
        return applyApprovedDreamProposal(payload.proposalId, request.reviewedBy || currentOwnerEmail(), requestCtx);
    }
    if (request.changeType === "workspace-resource.create") {
        const { applyWorkspaceResourceCreate } = await import("./workspace-resources-store.js");
        return applyWorkspaceResourceCreate(payload.input, request.reviewedBy || currentOwnerEmail(), requestCtx);
    }
    if (request.changeType === "workspace-resource.update") {
        const { applyWorkspaceResourceUpdate } = await import("./workspace-resources-store.js");
        return applyWorkspaceResourceUpdate(payload.id, payload.input, request.reviewedBy || currentOwnerEmail(), requestCtx);
    }
    if (request.changeType === "workspace-resource.delete") {
        const { applyWorkspaceResourceDelete } = await import("./workspace-resources-store.js");
        return applyWorkspaceResourceDelete(payload.id, request.reviewedBy || currentOwnerEmail(), requestCtx);
    }
...
export async function listIdentityState() {
    ...
    const owner = currentOwnerEmail();
    const orgId = currentOrgId();
    const filters = and(eq(schema.dispatchIdentityLinks.ownerEmail, owner), orgId
        ? eq(schema.dispatchIdentityLinks.orgId, orgId)
        : isNull(schema.dispatchIdentityLinks.orgId));
    const tokenFilters = and(eq(schema.dispatchLinkTokens.ownerEmail, owner), orgId
...
export async function resolveLinkedOwner(platform, externalUserId, options = {}) {
    if (!externalUserId)
        return null;
    const db = getDb();
    ...
    const rows = await db
        .select()
        .from(schema.dispatchIdentityLinks)
        .where(and(eq(schema.dispatchIdentityLinks.platform, platform), eq(schema.dispatchIdentityLinks.externalUserId, externalUserId), orgId
        ? eq(schema.dispatchIdentityLinks.orgId, orgId)
        : isNull(schema.dispatchIdentityLinks.orgId)))
        .orderBy(desc(schema.dispatchIdentityLinks.updatedAt))
        .limit(1);
    if (rows[0]?.ownerEmail)
        return rows[0].ownerEmail;
    if (!options.allowAnyOrgFallback || orgId)
        return null;
    ...
    const fallbackRows = await db
        .select()
        .from(schema.dispatchIdentityLinks)
        .where(and(eq(schema.dispatchIdentityLinks.platform, platform), eq(schema.dispatchIdentityLinks.externalUserId, externalUserId)))
        .orderBy(desc(schema.dispatchIdentityLinks.updatedAt))
        .limit(25);
...
export async function consumeLinkToken(input) {
    ...
    const tokenRows = await db
        .select()
        .from(schema.dispatchLinkTokens)
        .where(and(eq(schema.dispatchLinkTokens.platform, input.platform), eq(schema.dispatchLinkTokens.token, input.token)))
    ...
    const [existing] = await db
        .select()
        .from(schema.dispatchIdentityLinks)
        .where(and(eq(schema.dispatchIdentityLinks.platform, input.platform), eq(schema.dispatchIdentityLinks.externalUserId, input.externalUserId), tokenRow.orgId
...
export async function listOverview() {
    const [destinations, approvals, identities, audit, settings] = await Promise.all([
        listDestinations(),
    ...
    return {
        counts: {
            ...
            activeTokens: identities.tokens.filter((item) => !item.claimedAt && item.expiresAt > now()).length,
        },
