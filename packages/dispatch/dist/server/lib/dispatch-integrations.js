...
export function identityKeyForIncoming(incoming) {
...
function fallbackOwnerForIncoming(incoming) {
    const tenant = contextString(incoming.platformContext.teamId) ||
...
function configuredDefaultOwnerForIncoming(incoming) {
...
export async function beforeDispatchProcess(incoming, adapter) {
    const trimmed = incoming.text.trim();
    const commandText = contextString(incoming.platformContext.rawText) || trimmed;
    ...
    try {
        ...
        return {
            handled: true,
            responseText: error instanceof Error ? error.message : "Failed to link this account.",
        };
    }
}
//# sourceMappingURL=dispatch-integrations.js.map