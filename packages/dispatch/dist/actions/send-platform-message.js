...
import { getDestinationById, recordAudit, } from "../server/lib/dispatch-store.js";
function getAdapter(platform) {
    if (platform === "email")
        return emailAdapter();
    return platform === "slack" ? slackAdapter() : telegramAdapter();
}
function assertOutboundConfigured(platform) {
    ...
    if (platform === "email") {
        const hasProvider = !!(process.env.RESEND_API_KEY || process.env.SENDGRID_API_KEY);
...
export default defineAction({
    description: "Send a proactive message to a saved Slack, Telegram, or email destination.",
    schema: z.object({
    ...
    run: async ({ platform, destinationId, destination, threadRef, text }) => {
        const saved = destinationId
            ...
            : null;
        const resolvedPlatform = (saved?.platform || platform);
...
//# sourceMappingURL=send-platform-message.js.map