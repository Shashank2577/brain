...
export function useNavigationState(extensions) {
    ...
    const { data: navCommand } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(agentNativePath("/_agent-native/application-state/navigate"));
            if (!res.ok)
                return null;
    ...
    useEffect(() => {
        ...
        const resolvedPath = cmd.path || resolvePath(cmd.view, extensions) || "/overview";
        const path = cmd.view === "dreams" && cmd.dreamId && !resolvedPath.includes("?")
...
function resolveView(pathname, extensions) {
    ...
    if (pathname.startsWith("/dreams"))
        return "dreams";
    if (pathname.startsWith("/thread-debug"))
        return "thread-debug";
