 ...
 * Existing cookie auth is unaffected — `getSession()` already includes a
 * Bearer-token resolver path; the capability-registry plugin now layers a
 * mobile-JWT resolver on top of it so the dispatch shell can accept either
 * cookie or bearer JWT without changing any cookie behavior.
 *
