...
async function buildCloudflarePages() {
    ...
    for (const jsFile of allJsFiles) {
        ...
        code = code.replace(/\bfrom(\s*)(["'])node:([^"']+)\2/g, (_, ws, q, mod) => `from${ws}${q}${mod}${q}`);
        code = code.replace(/\bimport(\s*)(["'])node:([^"']+)\2/g, (_, ws, q, mod) => `import${ws}${q}${mod}${q}`);
        ...
        const relStub = path
            ...
            .replace(/\\/g, "/");
        code = code.replace(/["']virtual:react-router\/server-build["']/g, `"./${relStub}"`);
        ...
        if (isEntry) {
            const timerRestore = "if(__origSetInterval)globalThis.setInterval=__origSetInterval;";
            code = code.replace(/async fetch\(request,\s*env,\s*ctx\)\s*\{/, (match) => match + timerRestore);
        }
    ...
    console.log(`[deploy] Cloudflare Pages output written to dist/ (entry: ${(entrySize / 1024).toFixed(0)}KB, ${chunkCount} chunks, total: ${(totalSize / 1024 / 1024).toFixed(1)}MB)`);
}
const NODE_BUILTINS = [
    "assert",
    "async_hooks",
...
export function getNodeBuiltinNames() {
...
function createDanglingOptionalDepStubs() {
    ...
    while (true) {
        ...
        if (fs.existsSync(candidate))
            pnpmRoots.push(candidate);
        const parent = path.dirname(dir);
        if (parent === dir)
            break;
        dir = parent;
    }
    if (pnpmRoots.length === 0)
        return;
    let stubsCreated = 0;
                ...
                for (const { symlinkPath, pkgName } of candidates) {
                    ...
                    try {
                        ...
                        const stubPkgJson = {
                            name: pkgName,
                            version: "0.0.0-stub",
                            description: "Empty stub created by @agent-native/core deploy build to satisfy nitro's file tracer on platforms where this optional dep is not installed.",
                        };
                        fs.writeFileSync(path.join(resolvedTarget, "package.json"), JSON.stringify(stubPkgJson, null, 2));
                        stubsCreated++;
                    }
                    catch {
    ...
    if (stubsCreated > 0) {
        console.log(`[deploy] Created ${stubsCreated} stub package dir(s) for dangling optional deps (platform-specific binaries not installed on this host).`);
    }
}
/**
...
async function buildWithNitro() {
    ...
    const { createNitro, prepare, copyPublicAssets, build: nitroBuild, } = await import("nitro/builder");
...
`;
    ...
    if (preset.startsWith("cloudflare") || preset.startsWith("deno")) {
        ...
        const outputDir = nitro.options.output.serverDir || path.join(cwd, "dist", "_worker.js");
        const bareImports = new Set();
        function scanForBareImports(dir) {
            ...
            for (const mod of bareImports) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            const wsRequire = createRequire(path.resolve(cwd, "../../package.json"));
                    ...
                    function findNeededExports(dir) {
                        if (!fs.existsSync(dir))
                            return;
                        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                            const p = path.join(dir, entry.name);
                            ...
                            const re = new RegExp(`import\\{([^}]+)\\}from["']${escaped}["']`, "g");
                            for (const m2 of code.matchAll(re)) {
                                for (const part of m2[1].split(",")) {
                                    ...
                                    if (name && /^[a-zA-Z_$]/.test(name))
                                        neededExports.add(name);
                                }
                            }
                        }
                    }
                    findNeededExports(outputDir);
                    const entryCode = neededExports.size > 0
                        ? [
                            `import _mod from "${resolvedMod}";`,
                            `export default _mod;`,
                            ...Array.from(neededExports).map((n) => `export const ${n} = _mod.${n} ?? _mod?.default?.${n};`),
                        ].join("\n")
                    ...
                    function rewriteImports(dir) {
                        ...
                        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                            ...
                            const importPath = relPath.startsWith(".")
                                ? relPath
                                : "./" + relPath;
                            const re = new RegExp(`from["']${mod.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`, "g");
                            if (re.test(code)) {
                    ...
                    console.warn(`[deploy] Could not bundle: ${mod} (may not be needed at runtime)`);
                }
            }
        }
    }
    ...
    if (preset.startsWith("cloudflare")) {
        ...
        if (serverDir2) {
            ...
            if (fs.existsSync(chunksDir))
                scanDirs.push(chunksDir);
            if (fs.existsSync(libsDir))
                scanDirs.push(libsDir);
        }
        for (const scanDir of scanDirs) {
            ...
            for (const file of fs.readdirSync(scanDir)) {
                ...
                if (code.includes("setInterval") && !code.includes("__timer_shim__")) {
                    const shim = "/* __timer_shim__ */" +
                        "var __origSetInterval=globalThis.setInterval;" +
                        "globalThis.setInterval=function(){return{unref(){},ref(){},close(){}}};";
                    const restore = ";(function(){if(typeof __origSetInterval!=='undefined')globalThis.setInterval=__origSetInterval})();";
                    code = shim + code + "\n" + restore;
                    changed = true;
                }
                if (changed)
        ...
        const libsDir2 = path.join(serverDir2 || path.join(cwd, "dist", "_worker.js"), "_libs");
        if (fs.existsSync(libsDir2)) {
            ...
            for (const mod of NATIVE_STUBS) {
                ...
                if (!fs.existsSync(stubPath)) {
                    fs.writeFileSync(stubPath, `export default {}; export const watch = () => ({ close() {} });\n`);
                ...
                for (const filePath of referencingFiles) {
                    ...
                    if (importRe.test(code)) {
                        ...
                        fs.writeFileSync(filePath, code);
                        console.log(`[deploy] Rewrote ${mod} imports in _libs/${path.basename(filePath)}`);
                    }
                }
                // Also scan _chunks/ files (they import native deps too)
                const chunksDir2 = path.join(serverDir2 || path.join(cwd, "dist", "_worker.js"), "_chunks");
        ...
        console.log("[deploy] Patched bare Node imports, timer calls, and route finder for CF Workers");
    }
    await nitro.close();
