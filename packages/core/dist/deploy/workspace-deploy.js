...
function buildOneApp(workspaceRoot, app, preset, execFile, workspaceApps) {
    ...
    const workspaceGatewayUrl = process.env.VITE_WORKSPACE_GATEWAY_URL || workspaceBaseUrl();
    ...
    const env = {
        ...
        AGENT_NATIVE_WORKSPACE_APP_PUBLIC_PATHS: JSON.stringify(workspaceAppRouteAccess.publicPaths),
        AGENT_NATIVE_WORKSPACE_APP_PROTECTED_PATHS: JSON.stringify(workspaceAppRouteAccess.protectedPaths),
        VITE_AGENT_NATIVE_WORKSPACE_APP_AUDIENCE: workspaceAppAudience,
        VITE_AGENT_NATIVE_WORKSPACE_APP_PUBLIC_PATHS: JSON.stringify(workspaceAppRouteAccess.publicPaths),
        VITE_AGENT_NATIVE_WORKSPACE_APP_PROTECTED_PATHS: JSON.stringify(workspaceAppRouteAccess.protectedPaths),
        VITE_AGENT_NATIVE_WORKSPACE_APPS_JSON: JSON.stringify(workspaceApps),
        ...(workspaceGatewayUrl
            ? {
                WORKSPACE_GATEWAY_URL: process.env.WORKSPACE_GATEWAY_URL || workspaceGatewayUrl,
...
function writeCloudflareRoutingManifest(distDir, apps) {
    ...
    if (apps.includes("dispatch")) {
        ...
        include.push(...DISPATCH_WORKSPACE_ROOT_REDIRECTS.map(([from]) => `/${from}`));
        include.push("/apps/*");
...
function cloudflareRootRedirectPath(apps) {
...
function writeVercelBuildConfig(outputDir, apps) {
    ...
    if (apps.includes("dispatch")) {
        ...
        routes.push(vercelRedirect("/", "/dispatch/overview"), vercelRedirect("/dispatch", "/dispatch/overview"), vercelRedirect("/dispatch/", "/dispatch/overview"));
        for (const [from, to] of DISPATCH_WORKSPACE_ROOT_REDIRECTS) {
...
function netlifyAssetRedirectsFor(app, distDir) {
    ...
    return [
        ...
        ...netlifyPublicRootAssetPaths(app, path.join(distDir, NETLIFY_WORKSPACE_STATIC_DIR, app)).map((assetPath) => {
...
function assertNoReservedWorkspaceAppIds(apps) {
    const conflicts = apps.filter((app) => app !== "dispatch" && RESERVED_WORKSPACE_APP_IDS.has(app));
    if (conflicts.length === 0)
        return;
    throw new Error(`Workspace app id ${conflicts.map((id) => `"${id}"`).join(", ")} conflicts with reserved workspace routes. Choose a different app id.`);
}
function copyNetlifyFunctionIntoWorkspace(workspaceRoot, app, workspaceApps, staticDir) {
    const appDir = path.join(workspaceRoot, "apps", app);
...
function patchNetlifyFunctionEntry(functionDir, app, workspaceApps, staticDir) {
...
function netlifyPublicRootAssetPaths(app, staticDir) {
...
function writeWorkspaceAppManifests(workspaceRoot, distDir, apps, workspaceApps, preset) {
...
function readWorkspaceAppManifest(workspaceRoot, apps) {
    ...
    return apps
        ...
        const url = normalizeWorkspaceAppUrl(explicit?.url) ?? workspaceAppUrl(appPath);
        const audience = workspaceAppAudienceFromPackageJson(pkg) ??
            explicit?.audience ??
        ...
        const publicPaths = packageRouteAccess.publicPaths ?? explicit?.publicPaths ?? [];
        const protectedPaths = packageRouteAccess.protectedPaths ?? explicit?.protectedPaths ?? [];
        return {
            id: app,
...
function readExistingWorkspaceAppManifest(workspaceRoot) {
    const fromEnv = parseWorkspaceAppsJson(process.env[WORKSPACE_APPS_ENV_KEY]);
    const fromFile = readWorkspaceAppsFromFile(path.join(workspaceRoot, WORKSPACE_APPS_MANIFEST_DIR, WORKSPACE_APPS_MANIFEST_FILE)) ??
        readWorkspaceAppsFromFile(path.join(workspaceRoot, WORKSPACE_APPS_MANIFEST_FILE));
    const apps = fromEnv ?? fromFile ?? [];
    return new Map(apps.map((app) => [app.id, app]));
}
function parseWorkspaceAppsJson(raw) {
    if (!raw)
        return null;
    try {
...
function readWorkspaceAppsFromFile(file) {
...
function parseWorkspaceAppsManifest(parsed) {
    ...
    const apps = rawApps
        ...
        const audience = entry.audience === undefined
            ? undefined
            : normalizeWorkspaceAppAudience(entry.audience);
        const publicPaths = normalizeWorkspaceAppPathList(entry.publicPaths);
        const protectedPaths = normalizeWorkspaceAppPathList(entry.protectedPaths);
        return {
            id,
            ...(url ? { url } : {}),
            ...(audience ? { audience } : {}),
...
function workspaceBaseUrl() {
    const gatewayOrigin = process.env.WORKSPACE_GATEWAY_URL || process.env.VITE_WORKSPACE_GATEWAY_URL;
    const publicGatewayOrigin = normalizeOrigin(gatewayOrigin);
    const gatewayFallback = publicGatewayOrigin && !isLoopbackOrigin(publicGatewayOrigin)
        ? gatewayOrigin
        : null;
...
function workspaceOAuthOrigin(workspaceGatewayUrl) {
    ...
    return (normalizeOrigin(process.env.VITE_WORKSPACE_OAUTH_ORIGIN) ||
...
function normalizeWorkspaceAppUrl(value) {
...
function assertWorkspaceDeployProductionEnv(opts) {
    if (!isProductionWorkspaceDeploy(opts))
        return;
    if (process.env.A2A_SECRET?.trim())
        return;
    const providerHint = opts.preset === "netlify"
...
function workspaceAppAudienceForApp(workspaceApps, app) {
    return (workspaceApps.find((entry) => entry.id === app)?.audience ??
        DEFAULT_WORKSPACE_APP_AUDIENCE);
}
function workspaceAppRouteAccessForApp(workspaceApps, app) {
...
function compareWorkspaceAppIds(a, b) {
    if (a === "dispatch")
        return -1;
    if (b === "dispatch")
        return 1;
...
//# sourceMappingURL=workspace-deploy.js.map