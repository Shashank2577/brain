import { createError, defineEventHandler, getRouterParam, getHeader, setResponseStatus, } from "h3";
...
export const putState = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const key = safeKey(String(getRouterParam(event, "key")));
...
export const deleteState = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const key = safeKey(String(getRouterParam(event, "key")));
    const requestSource = getHeader(event, "x-request-source") || undefined;
...
export const listComposeDrafts = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const items = await appStateList(sessionId, "compose-");
    return items.map((item) => item.value);
});
/** Get a single compose draft */
export const getComposeDraft = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
...
export const putComposeDraft = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const id = getRouterParam(event, "id");
    const body = await readBody(event);
...
export const deleteComposeDraft = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const id = getRouterParam(event, "id");
    const requestSource = getHeader(event, "x-request-source") || undefined;
...
export const deleteAllComposeDrafts = defineEventHandler(async (event) => {
    const sessionId = await getSessionId(event);
    const requestSource = getHeader(event, "x-request-source") || undefined;
