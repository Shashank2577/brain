 ...
 */
export const getCollabState = defineEventHandler(async (event) => {
 ...
 */
export const postCollabUpdate = defineEventHandler(async (event) => {
    const docId = getRouterParam(event, "docId");
    if (!docId) {
        setResponseStatus(event, 400);
        return { error: "docId required" };
 ...
 */
export const postCollabText = defineEventHandler(async (event) => {
    const docId = getRouterParam(event, "docId");
    if (!docId) {
        setResponseStatus(event, 400);
        return { error: "docId required" };
    ...
    const result = await manager.applyText(docId, text, fieldName ?? "content", requestSource ?? "agent");
 ...
 */
export const postCollabSearchReplace = defineEventHandler(async (event) => {
    const docId = getRouterParam(event, "docId");
    if (!docId) {
        setResponseStatus(event, 400);
        return { error: "docId required" };
    ...
    const result = await doSearchAndReplace(docId, find, replace ?? "", requestSource ?? "agent");
