 ...
 */
export function agentEnterDocument(docId, metadata) {
 ...
 */
export function agentLeaveDocument(docId) {
 ...
 */
export function agentUpdateSelection(docId, selection) {
    const map = getDocAwareness(docId);
    ...
    if (existing) {
        try {
            parsed = JSON.parse(existing.state);
        }
        catch {
 ...
 */
export async function agentApplyEditsIncrementally(docId, edits, options) {
...
export async function agentApplyPatchesIncrementally(docId, fieldName, patches, options) {
    ...
    try {
        ...
        try {
            ...
            applyPatchOps = mod.applyPatchOps;
        }
        catch {
            throw new Error("applyPatchOps is not available yet — Phase 1 must complete first");
        }
