 ...
 */
export const postCollabJson = defineEventHandler(async (event) => {
    ...
    await manager.applyJson(docId, json, fieldName ?? "data", type ?? (Array.isArray(json) ? "array" : "map"), requestSource ?? "agent");
    return { ok: true };
});
/**
 ...
 */
export const postCollabPatch = defineEventHandler(async (event) => {
    const docId = getRouterParam(event, "docId");
    if (!docId) {
        setResponseStatus(event, 400);
        return { error: "docId required" };
    ...
    await manager.applyPatchOps(docId, ops, fieldName ?? "data", requestSource ?? "agent");
    return { ok: true };
});
/**
 ...
 */
export const getCollabJson = defineEventHandler(async (event) => {
    const docId = getRouterParam(event, "docId");
    if (!docId) {
        setResponseStatus(event, 400);
        return { error: "docId required" };
    ...
    const fieldName = query.fieldName ?? "data";
    const data = await manager.getJson(docId, fieldName);
    return { docId, data };
});
//# sourceMappingURL=struct-routes.js.map