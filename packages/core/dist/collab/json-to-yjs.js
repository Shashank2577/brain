 ...
 */
export function seedYDocFromJson(doc, fieldName, json, type) {
    doc.transact(() => {
        if (type === "map") {
 ...
 */
export function yMapToJson(ymap) {
    const result = {};
 ...
 * Recurses into nested Y.Map/Y.Array.
 */
export function yArrayToJson(yarray) {
    const result = [];
 ...
 */
export function yDocToJson(doc, fieldName) {
    const existing = doc.share.get(fieldName);
    if (existing instanceof Y.Array)
        return yArrayToJson(existing);
    if (existing instanceof Y.Map)
        return yMapToJson(existing);
    return {};
}
 ...
 */
export function applyJsonDiff(doc, fieldName, newJson, origin) {
...
function diffArrayById(yarray, newArr) {
    ...
    for (let i = 0; i < newArr.length; i++) {
        ...
        const currentId = currentItem instanceof Y.Map ? currentItem.get("id") : undefined;
 ...
 */
export function applyJsonPatch(doc, fieldName, ops, origin) {
    let update = new Uint8Array(0);
    const handler = (u) => {
        update = u;
    };
    ...
    switch (patchOp.op) {
        case "set": {
            ...
            if (parent instanceof Y.Map) {
                parent.set(key, jsonToYType(patchOp.value));
            }
...
function navigateToParent(doc, fieldName, segments) {
    if (segments.length === 0)
        return { parent: null, key: undefined };
 ...
 */
export function initYDocWithJson(fieldName, json, type) {
    const doc = new Y.Doc();
...
//# sourceMappingURL=json-to-yjs.js.map