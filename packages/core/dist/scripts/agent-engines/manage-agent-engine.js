    ...
    parameters: {
        ...
        properties: {
            action: {
                ...
                description: '"list" — show available engines and current global selection. "set" — change the active global engine/model. "test" — send a trivial prompt to verify connectivity. "get-app-default" — show this app/template default. "set-app-default" — set this app/template default. "reset-app-default" — clear this app/template default.',
            },
            engine: {
                type: "string",
                description: 'Engine name (e.g. "builder", "anthropic", "ai-sdk:openai", "ai-sdk:google"). Required for "set" and "set-app-default", optional for "test" (defaults to "anthropic").',
            },
            model: {
                type: "string",
                description: "Model ID (e.g. 'gpt-5.5', 'claude-sonnet-4-6', 'gemini-3-1-pro'). Required for \"set-app-default\"; optional for \"set\" and \"test\" where it defaults to the engine's default model.",
            },
            appId: {
                type: "string",
                description: "App/template id whose default model should be managed. Defaults to the current app.",
            },
        },
        required: ["action"],
    },
...
async function runGetAppDefault(args) {
...
async function runSetAppDefault(args) {
...
async function runResetAppDefault(args) {
    const appId = resolveAppId(args);
    if (!appId)
        return "Error: appId is required";
    const ctx = currentContext();
...
//# sourceMappingURL=manage-agent-engine.js.map