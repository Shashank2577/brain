...
async function discoverColumnsSqlite(client) {
    ...
    const tables = tablesResult.rows.map((r) => (r.name ?? r[0]));
    const result = [];
...
export default async function dbCheckScoping(args) {
...
Options:
  ...
  --help            Show this help message`);
    ...
    if (withIssues.length > 0) {
        ...
        console.log();
        console.log(`${withIssues.length} table(s) lack scoping columns. Raw db-* tools ` +
...
//# sourceMappingURL=check-scoping.js.map