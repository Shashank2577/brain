...
export async function run(args, context, selfAppId) {
...
function getA2ATaskTimeoutTaskId(err) {
    ...
    if (candidate?.name === "A2ATaskTimeoutError" &&
...
export function expandRelativeUrls(text, agentUrl) {
    if (!text || !agentUrl)
        return text;
