import { executeExistingCodeAgentRun, executePendingCodeAgentApproval, } from "../cli/code-agent-executor.js";
import { appendCodeAgentTranscriptEvent, getCodeAgentRunRecord, isActiveCodeAgentRun, queueCodeAgentFollowUp, updateCodeAgentRunRecord, } from "../cli/code-agent-runs.js";
import { getBackgroundAgentRun, listBackgroundAgentRuns, listBackgroundAgentTranscriptEvents, toBackgroundAgentRun, } from "./background-run.js";
export function createCompositeBackgroundAgentController(controllers) {
    const resolveControllerForRun = async (runId) => {
    ...
    return {
        async list(options) {
            const groups = await Promise.all(controllers.map((controller) => Promise.resolve(controller.list(options)).catch(() => [])));
            return groups
                .flat()
...
export function createLocalCodeBackgroundAgentController() {
    return {
...
export const localCodeBackgroundAgentController = createLocalCodeBackgroundAgentController();
async function sendLocalCodeBackgroundAgentFollowUp(input) {
    ...
    const shouldQueue = activeRun.status === "queued" || isActiveCodeAgentRun(activeRun);
...
async function controlLocalCodeBackgroundAgentRun(input) {
    const run = getCodeAgentRunRecord(input.runId);
    if (!run) {
        return missingRunResult(input.runId);
    }
    switch (input.command) {
        ...
        case "retry": {
            ...
            void executeExistingCodeAgentRun(run.id, {
                ...
                appendCodeAgentTranscriptEvent({
                    runId: run.id,
                    kind: "status",
                    message: `Background ${input.command} failed: ${err instanceof Error ? err.message : String(err)}`,
                    metadata: {
        ...
        default: {
            const exhaustive = input.command;
            return {
                ok: false,
                runId: run.id,
                run: currentBackgroundRun(run.id),
...
function stopLocalCodeBackgroundAgentRun(runId) {
    ...
    appendCodeAgentTranscriptEvent({
        runId,
        kind: "status",
        message: "Stop requested for Agent-Native Code run. No process signal was sent.",
        metadata: {
            source: "background-agent-controller",
...
function missingRunResult(runId) {
    return {
        ok: false,
        runId,
        run: null,
...
//# sourceMappingURL=background-controller.js.map