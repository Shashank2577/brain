 ...
 */
import { awaitBootstrap, markDefaultPluginProvided, } from "./framework-request-handler.js";
import { getSession } from "./auth.js";
import { captureRouteError, initServerSentry, isServerSentryEnabled, setSentryRequestContext, setSentryUserForRequest, } from "./sentry.js";
...
export function createSentryPlugin() {
    return async (nitroApp) => {
        ...
        nitroApp.hooks?.hook?.("request", async (event) => {
            if (!shouldResolveSession(readRoute(event)))
                return;
            try {
 ...
 */
export const defaultSentryPlugin = createSentryPlugin();
