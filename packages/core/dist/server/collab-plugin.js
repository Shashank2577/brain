...
import { getCollabState, postCollabUpdate, postCollabText, postCollabSearchReplace, } from "../collab/routes.js";
import { postCollabJson, getCollabJson, postCollabPatch, } from "../collab/struct-routes.js";
...
export function createCollabPlugin(options = {}) {
    ...
    return async (nitroApp) => {
        ...
        // `/abc-123/state`.
        getH3App(nitroApp).use(`${P}/collab`, defineEventHandler(async (event) => {
            ...
            return runWithRequestContext({ userEmail, orgId }, async () => {
                ...
                if (resourceType) {
                    ...
                    const isWrite = (action === "update" && method === "POST") ||
        ...
        if (autoSeed) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    const { rows } = await client.execute(`SELECT ${idColumn}, ${seedColumn} FROM ${table}`);
                    for (const row of rows) {
                        const docId = row[idColumn];
                        const exists = await hasCollabState(docId);
                        if (exists)
                            continue;
                        if (isJson) {
                            const raw = row[seedColumn] ?? "{}";
                            try {
                                const parsed = JSON.parse(raw);
                                const inferredType = Array.isArray(parsed)
                                    ? "array"
                                    : "map";
...
//# sourceMappingURL=collab-plugin.js.map