    ...
    return defineEventHandler(async (event) => {
        ...
        if (providerPref === "gemini") {
            ...
            if (!geminiKey) {
                setResponseStatus(event, 400);
                return {
                    error: "Gemini is selected but GEMINI_API_KEY is not configured. Add it in Settings → API Keys, or change the provider preference.",
                };
            }
            try {
                ...
                setResponseStatus(event, 502);
                return {
                    error: `Gemini transcription failed: ${err?.message ?? String(err)}`,
                };
            }
        }
        ...
        if (providerPref === "groq") {
            ...
            if (!groqKey) {
                setResponseStatus(event, 400);
                return {
                    error: "Groq is selected but GROQ_API_KEY is not configured. Add it in Settings → API Keys, or change the provider preference.",
                };
            }
    ...
    try {
        ...
        return {
            error: err?.name === "AbortError"
                ...
                : `Could not reach ${provider.name}: ${err?.message ?? err}`,
        };
    }
    finally {
