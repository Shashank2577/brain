 ...
 */
const BUILTIN_AGENTS = TEMPLATES.filter((template) => (!template.hidden || template.defaultAgent) && !!template.prodUrl).map((template) => ({
    id: template.name,
    name: template.label,
 ...
 */
export async function discoverAgents(selfAppId) {
    ...
    try {
        const { resourceList, resourceGet, SHARED_OWNER } = await import("../resources/store.js");
        const { parseRemoteAgentManifest, REMOTE_AGENT_RESOURCE_PREFIXES } = await import("../resources/metadata.js");
        const resources = [];
