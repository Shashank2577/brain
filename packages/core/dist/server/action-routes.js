...
function handleOptionsRequest(event) {
    ...
    const allowedOrigin = getAllowedCorsOrigin(typeof origin === "string" ? origin : undefined);
 ...
 */
export function mountActionRoutes(nitroApp, actions, options) {
    const mounted = [];
    for (const [name, entry] of Object.entries(actions)) {
        ...
        getH3App(nitroApp).use(routePath, defineEventHandler(async (event) => {
            ...
            const fromToolBridge = getHeader(event, "x-agent-native-tool-bridge") === "1";
            ...
            return runWithRequestContext({ userEmail, userName, orgId, timezone }, async () => {
                ...
                try {
                    ...
                    const isReadOnly = typeof entry.readOnly === "boolean"
                        ? entry.readOnly
                        : method === "GET";
                    if (!isReadOnly) {
                        try {
                    ...
                    setResponseStatus(event, msg.startsWith("Invalid action parameters")
                        ? 400
    ...
    if (mounted.length > 0 && process.env.DEBUG)
        console.log(`[action-routes] Mounted ${mounted.length} action route(s): ${mounted.join(", ")}`);
}
