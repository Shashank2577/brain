...
function addNormalizedOrigin(out, raw, options) {
    const origin = normalizeOrigin(raw);
    if (!origin)
        return;
    if (!options.allowLoopback && isLoopbackOrigin(origin))
        return;
    out.add(origin);
}
function firstOriginFromEnv(keys, options) {
...
function getDefaultOAuthRedirectUrl(event, path) {
    const cleanPath = path.startsWith("/") ? path : `/${path}`;
    if (isWorkspaceOAuthCallbackRelayEnabled() &&
 ...
 */
export function isAllowedOAuthRedirectUri(candidate, event) {
 ...
 * desktop flows).
 */
export function resolveOAuthRedirectUri(event, defaultPath = "/_agent-native/google/callback") {
 ...
 */
export function decodeOAuthState(stateParam, fallbackUri) {
    if (stateParam) {
        try {
