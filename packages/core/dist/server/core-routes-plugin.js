...
async function trackBuilderLifecycle(event, name, userEmail, properties = {}) {
    if (!userEmail)
        return;
...
function readBuilderConnectOwnerCookie(event) {
    return verifyBuilderConnectTokenAndGetOwner(getCookie(event, BUILDER_CONNECT_OWNER_COOKIE));
}
function setBuilderConnectOwnerCookie(event, ownerEmail) {
    setCookie(event, BUILDER_CONNECT_OWNER_COOKIE, signBuilderConnectToken(ownerEmail), {
        httpOnly: true,
        sameSite: "lax",
...
function clearBuilderConnectOwnerCookie(event) {
 ...
 */
export function resolveLegacyToolsRedirect(rawPath, search) {
 ...
 */
export function createCoreRoutesPlugin(options = {}) {
 ...
 * ```
 */
export const defaultCoreRoutesPlugin = createCoreRoutesPlugin();
//# sourceMappingURL=core-routes-plugin.js.map