 ...
 */
export function markDefaultPluginProvided(nitroApp, stem) {
    if (!nitroApp || !stem)
        return;
    const existing = nitroApp[PROVIDED_PLUGIN_STEMS_KEY];
    const provided = existing ?? new Set();
    provided.add(stem);
 ...
 */
export function getH3App(nitroApp) {
    if (!nitroApp)
        throw new Error("getH3App: nitroApp is required");
 ...
 */
export async function awaitBootstrap(nitroApp) {
    if (!nitroApp || IN_BOOTSTRAP.has(nitroApp))
        return;
 ...
 */
export function trackPluginInit(nitroApp, promise) {
    if (!nitroApp)
        return;
 ...
 */
export async function awaitPluginsReady(nitroApp) {
...
async function bootstrapDefaultPlugins(nitroApp) {
    ...
    try {
            ...
            if (ws && Object.keys(ws.plugins).length > 0) {
                try {
                    ...
                    if (process.env.DEBUG) {
                        console.log(`[agent-native] Workspace core ${ws.packageName} provides plugin slots: ${Object.keys(workspaceImpls).join(", ")}`);
                    }
                }
                catch (e) {
                    const msg = e.message ?? "";
        ...
        if (process.env.DEBUG)
            console.log(`[agent-native] Auto-mounting ${missing.length} default plugin(s): ${missing.join(", ")}`);
...
export async function loadWorkspaceCoreServer(packageName, packageDir) {
    ...
    try {
        ...
        const anchor = pathToFileURL(path.join(packageDir, "package.json")).toString();
