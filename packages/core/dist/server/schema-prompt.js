 ...
 */
import { getDbExec, getDatabaseUrl, isPostgres, } from "../db/client.js";
...
export function invalidateSchemaPromptCache() {
    _cache = null;
}
// ─── Formatting ─────────────────────────────────────────────────────────────
...
function formatTable(table) {
    ...
    const cols = table.columns.map((c) => {
        ...
        if (c.name === "owner_email")
            flags.push("user-scope");
        if (c.name === "org_id")
            flags.push("org-scope");
