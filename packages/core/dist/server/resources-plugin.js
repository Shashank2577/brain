 ...
 */
export function createResourcesPlugin() {
    return async (nitroApp) => {
        ...
        getH3App(nitroApp).use("/_agent-native/resources", defineEventHandler(async (event) => {
            ...
            if (!subPath || subPath === "") {
                if (method === "GET")
                    return handleListResources(event);
                if (method === "POST")
            ...
            if (method === "GET")
                return handleGetResource(event);
            if (method === "PUT")
                return handleUpdateResource(event);
            if (method === "DELETE")
                return handleDeleteResource(event);
            setResponseStatus(event, 405);
            return { error: "Method not allowed" };
        }));
    };
 ...
 * ```
 */
export const defaultResourcesPlugin = createResourcesPlugin();
//# sourceMappingURL=resources-plugin.js.map