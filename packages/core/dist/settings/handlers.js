...
export const getSettingHandler = defineEventHandler(async (event) => {
...
export const putSettingHandler = defineEventHandler(async (event) => {
    const key = safeKey(String(getRouterParam(event, "key")));
...
export const deleteSettingHandler = defineEventHandler(async (event) => {
    const key = safeKey(String(getRouterParam(event, "key")));
    const requestSource = getHeader(event, "x-request-source") || undefined;
