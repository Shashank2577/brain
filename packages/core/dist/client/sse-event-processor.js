...
function isMissingCredentialText(message, errorCode) {
...
function dispatchActivityClear(tabId) {
    if (typeof window === "undefined")
        return;
    window.dispatchEvent(new CustomEvent("agent-chat:activity-clear", {
        detail: { tabId },
    }));
}
/**
 ...
 */
export function processEvent(ev, content, toolCallCounter, tabId) {
    if (ev.type === "clear") {
    ...
    if (ev.type === "tool_start") {
        ...
        if (typeof window !== "undefined") {
            window.dispatchEvent(new CustomEvent("agent-native:tool-start", {
    ...
    if (ev.type === "tool_done") {
        if (typeof window !== "undefined") {
            window.dispatchEvent(new CustomEvent("agent-native:tool-done", {
...
export async function* readSSEStream(body, content, toolCallCounter, tabId, onSeq, runId) {
    ...
    const activityTrail = [];
    const withStreamMetadata = (r) => {
        if (!runId && activityTrail.length === 0)
            return r;
        const metadata = (r.metadata ?? {});
        const custom = metadata.custom && typeof metadata.custom === "object"
            ? metadata.custom
            : {};
        const runError = runId && custom.runError && typeof custom.runError === "object"
            ? {
                ...custom.runError,
                runId,
            }
    ...
    try {
        while (true) {
            const { done, value } = await readChunkWithProgressTimeout(reader, lastMeaningfulEventAt);
            if (done)
                break;
 ...
 */
export async function readSSEStreamRaw(body, content, toolCallCounter, tabId, onUpdate) {
    const reader = body.getReader();
    const decoder = new TextDecoder();
    let buf = "";
    let lastMeaningfulEventAt = Date.now();
    try {
        while (true) {
            ...
            for (const line of lines) {
                ...
                sawDataEvent = true;
                lastMeaningfulEventAt = Date.now();
                const { action, autoContinue } = processEvent(ev, content, toolCallCounter, tabId);
                if (action === "yield" ||
                    action === "done" ||
                    action === "error" ||
                    action === "missing_api_key") {
