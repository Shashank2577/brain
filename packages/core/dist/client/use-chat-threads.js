...
export function useChatThreads(apiUrl = agentNativePath("/_agent-native/agent-chat"), storageKey, scope) {
    ...
    seedAt) => {
        const stamp = typeof seedAt === "number" && Number.isFinite(seedAt)
            ? seedAt
            : Date.now();
        const optimistic = {
            id,
    ...
    const generateTitle = useCallback(async (threadId, message) => {
        try {
    ...
    const forkThread = useCallback(async (sourceId, sourceSnapshot) => {
        const id = crypto.randomUUID();
        const fallbackForkFromSnapshot = async (source) => {
            ...
            if (!createRes.ok)
                return null;
            const saveRes = await fetch(`${apiUrl}/threads/${encodeURIComponent(id)}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.error(`[chat] fork failed for ${sourceId}: ${res.status} ${res.statusText}`);
    ...
    const searchThreads = useCallback(async (query) => {
        try {
            const res = await fetch(`${apiUrl}/threads?q=${encodeURIComponent(query)}`);
            if (!res.ok)
                return [];
            const data = await res.json();
            return data.threads ?? [];
