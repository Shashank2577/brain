...
const BUILD_APP_OR_AGENT_RE = /\b(?:build|create|make|scaffold|generate)\b[^.!?\n]*?\b(?:agent[-\s]native\s+)?(?:workspace\s+)?(?:app|agent)\b/i;
/**
 ...
 */
export function isBuildAppOrAgentRequest(text) {
    const t = (text ?? "").trim();
    if (!t)
        return false;
 ...
 */
export function tryDelegateBuildRequestToBuilder(text) {
    if (!isInBuilderFrame())
        return false;
    if (!isBuildAppOrAgentRequest(text))
        return false;
    return sendToBuilderChat({ message: (text ?? "").trim(), submit: true });
}
//# sourceMappingURL=builder-frame.js.map