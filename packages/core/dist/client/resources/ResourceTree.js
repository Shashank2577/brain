...
function getFileIcon(node) {
    if (node.kind === "agent") {
        return (_jsx(IconMessageChatbot, { className: "h-3.5 w-3.5 shrink-0 text-muted-foreground" }));
    }
    ...
    if (node.kind === "job") {
        return (_jsx(IconClockHour3, { className: "h-3.5 w-3.5 shrink-0 text-muted-foreground" }));
    }
    const name = node.name;
    const ext = name.split(".").pop()?.toLowerCase() ?? "";
    const iconClass = "h-3.5 w-3.5 shrink-0 text-muted-foreground";
    if (ext === "md" || ext === "mdx")
        return _jsx(IconFileText, { className: iconClass });
    if (["ts", "tsx", "js", "jsx", "json", "css", "html", "py", "sh"].includes(ext))
        return _jsx(IconFileCode, { className: iconClass });
    if (["png", "jpg", "jpeg", "gif", "svg", "webp", "ico"].includes(ext))
        return _jsx(IconPhoto, { className: iconClass });
    return _jsx(IconFile, { className: iconClass });
}
...
function InlineInput({ depth, onConfirm, onCancel, }) {
    ...
    return (_jsxs("div", { className: "flex items-center gap-1 px-1.5 py-0.5", style: { paddingLeft: depth * 16 + 6 + 16 }, children: [_jsx(IconFile, { className: "h-3.5 w-3.5 shrink-0 text-muted-foreground" }), _jsx("input", { ref: inputRef, value: value, onChange: (e) => setValue(e.target.value), onKeyDown: (e) => {
...
// ─── ResourceTree ───────────────────────────────────────────────────────────
export function ResourceTree({ tree, selectedId, onSelect, onCreateFile, onCreateFolder, onDelete, onDrop, title = "Files", titleTooltip, isLoading = false, deletingId = null, readOnly = false, headingHint, }) {
    ...
    const handleStartCreate = useCallback((parentPath, type) => {
    ...
    const handleConfirmCreate = useCallback((name) => {
        if (!creating)
            return;
