...
async function loadSchema(target) {
    const key = `${target.endpoint}|${target.slug}`;
    let pending = schemaCache.get(key);
    if (pending)
        return pending;
    pending = (async () => {
        const res = await fetch(`${target.endpoint}/api/forms/public/${encodeURIComponent(target.slug)}`, { headers: { Accept: "application/json" } });
        if (!res.ok)
            throw new Error(`form fetch ${res.status}`);
        const body = (await res.json());
        const field = body.fields.find((f) => f.type === "textarea") ??
