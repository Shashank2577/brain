    ...
    try {
        ...
        if (match[2]) {
            if (typeof atob === "function") {
                return decodeURIComponent(Array.from(atob(payload), (char) => `%${char.charCodeAt(0).toString(16).padStart(2, "0")}`).join(""));
            }
            return null;
        }
...
function authErrorReasonFromMessage(message) {
    const msg = message.toLowerCase();
...
function isMissingCredentialMessage(message) {
    const msg = message.toLowerCase();
    return (msg.includes("apikey") ||
    ...
    return {
        async *run({ messages, abortSignal, runConfig }) {
            ...
            const rawMessageText = lastUserMsg?.content
                ...
                .map((p) => p.text)
                .join("\n") ?? "";
            const runConfigRequestMode = runConfig?.custom &&
                typeof runConfig.custom === "object" &&
                "requestMode" in runConfig.custom
                ? runConfig.custom.requestMode
                : undefined;
            const requestMode = runConfigRequestMode === "act" || runConfigRequestMode === "plan"
                ...
                : execModeRef?.current === "plan"
                    ...
                    : execModeRef?.current === "build"
                        ? "act"
                        : undefined;
            const withRequestModeMetadata = (result) => {
                if (!requestMode)
                    return result;
            ...
            const tryRecoverAuthOnce = async () => {
                if (authRecoveryAttempted || abortSignal.aborted)
                    return false;
                authRecoveryAttempted = true;
                try {
                    const sessionRes = await fetch(safeAgentNativePath("/_agent-native/auth/session"), {
                        method: "GET",
            ...
            try {
                ...
                const reconnectCurrentRun = async function* () {
                    ...
                    for (let attempt = 0; attempt < MAX_RECONNECT_ATTEMPTS; attempt++) {
                        try {
                            ...
                            if (!reconnectRes.ok || !reconnectRes.body) {
                                lastReconnectError = new Error(`Reconnect failed: ${reconnectRes.status}`);
                                captureChatClientError(lastReconnectError, "reconnect-current-response", {
                    ...
                    if (lastReconnectError && !reconnectErrorCaptured) {
                        captureChatClientError(lastReconnectError, "reconnect-current-failed");
                    }
                    return false;
                };
                ...
                const reconnectActiveRunForThread = async function* () {
                    ...
                    for (let attempt = 0; attempt < MAX_RECONNECT_ATTEMPTS; attempt++) {
                        try {
                            ...
                            if (!activeRes.ok) {
                                ...
                                captureChatClientError(lastActiveRunError, "reconnect-active-response", { status: activeRes.status, attempt });
                                return false;
                            }
                    ...
                    if (lastActiveRunError) {
                        captureChatClientError(lastActiveRunError, "reconnect-active-failed");
                    }
                    return false;
                };
                ...
                const prepareAutoContinuation = (signal) => {
                    ...
                    if (signal.reason === "loop_limit") {
                        ...
                        if (stalledTransientContinuationAttempts >
...
//# sourceMappingURL=agent-chat-adapter.js.map