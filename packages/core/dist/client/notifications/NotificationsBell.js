...
export function NotificationsBell({ pollMs = POLL_MS_DEFAULT, className, browserNotifications = false, emptyTitle = "No app notifications yet.", emptyDescription, }) {
    ...
    const loadItems = useCallback(async () => {
        try {
            const res = await fetch(agentNativePath("/_agent-native/notifications?limit=20"));
            if (!res.ok)
                return;
    ...
    const refresh = useCallback(async () => {
        if (browserNotifications) {
            try {
                const res = await fetch(agentNativePath("/_agent-native/notifications?unread=true&limit=20"));
                if (!res.ok)
                    return;
                const rows = (await res.json());
        ...
        try {
            const res = await fetch(agentNativePath("/_agent-native/notifications/count"));
            if (!res.ok)
                return;
...
function SeverityBadge({ severity }) {
    ...
    return (_jsx("span", { className: `rounded px-1.5 py-0.5 text-[10px] font-medium ${color}`, children: severity }));
}
