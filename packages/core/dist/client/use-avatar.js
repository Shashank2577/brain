...
async function fetchAvatar(email) {
    ...
    if (_inFlight.has(email))
        return _inFlight.get(email);
    const p = fetch(agentNativePath(`/_agent-native/avatar/${encodeURIComponent(email)}`))
...
export function invalidateAvatarCache(email) {
    _cache.delete(email);
    _inFlight.delete(email);
}
/** Returns the avatar data URL for a given email, or null if none is set. */
export function useAvatarUrl(email) {
    const [url, setUrl] = useState(email ? (_cache.get(email) ?? null) : null);
    useEffect(() => {
        if (!email)
            return;
        let cancelled = false;
        ...
        if (!_listeners.has(email))
            _listeners.set(email, new Set());
