...
export function useCreateOrg() {
    ...
    return useMutation({
        mutationFn: (name) => apiFetch(ORG_BASE, {
            method: "POST",
...
export function useAcceptInvitation() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (invitationId) => apiFetch(`${ORG_BASE}/invitations/${invitationId}/accept`, {
            method: "POST",
        }),
        onSuccess: async () => {
...
export function useRemoveMember() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (email) => apiFetch(`${ORG_BASE}/members/${encodeURIComponent(email)}`, {
            method: "DELETE",
        }),
        onSuccess: async () => {
            await qc.invalidateQueries({ queryKey: ["org-members"] });
...
export function useSwitchOrg() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (orgId) => apiFetch(`${ORG_BASE}/switch`, {
            method: "PUT",
...
export function useJoinByDomain() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (orgId) => apiFetch(`${ORG_BASE}/join-by-domain`, {
            method: "POST",
            body: JSON.stringify({ orgId }),
        }),
        onSuccess: async () => {
...
export function useSetOrgDomain() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (domain) => apiFetch(`${ORG_BASE}/domain`, {
            method: "PUT",
...
export function useSetA2ASecret() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (secret) => apiFetch(`${ORG_BASE}/a2a-secret`, {
            method: "PUT",
...
export function useSyncA2ASecret() {
    return useMutation({
        mutationFn: (vars) => apiFetch(`${ORG_BASE}/a2a-secret/sync`, {
            method: "POST",
            body: JSON.stringify({
                signSecret: vars && "signSecret" in vars ? vars.signSecret : undefined,
            }),
        }),
    });
}
