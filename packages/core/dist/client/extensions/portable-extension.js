...
export function getAgentNativeExtensionManifest(extension) {
    return {
        slots: firstList(extension.manifest?.slots, extension.slots),
        requestedActions: firstList(extension.manifest?.requestedActions, extension.requestedActions),
        requestedCommands: firstList(extension.manifest?.requestedCommands, extension.requestedCommands),
        storageScopes: firstList(extension.manifest?.storageScopes, extension.storageScopes),
    };
}
export function isAgentNativeExtensionAllowedInSlot(extension, slotId) {
    if (!slotId)
        return true;
...
function storageScope(options, context) {
...
function bucketKey(namespace, extensionId, scope, collection) {
    return [
        "agent-native",
        "extension-data",
...
function readBucket(storage, key) {
...
function writeBucket(storage, key, value) {
    storage.setItem(key, JSON.stringify(value));
}
function listLocalStorageBuckets(storage, namespace, extensionId, collection) {
    const prefix = [
        "agent-native",
        "extension-data",
        encodedSegment(namespace),
    ...
    return {
        list(collection, options, context) {
            ...
            const keys = scope === "all"
                ? listLocalStorageBuckets(storage, namespace, context.extensionId, collection)
                : [bucketKey(namespace, context.extensionId, scope, collection)];
            const rows = keys.flatMap((key) => Object.values(readBucket(storage, key)));
        ...
        get(collection, id, options, context) {
            ...
            if (rows instanceof Promise) {
                return rows.then((resolved) => resolved.find((row) => row.id === id) ?? null);
            }
...
async function resolveHttpStorageHeaders(headers, context) {
    return typeof headers === "function" ? headers(context) : headers;
}
async function readHttpStorageResponse(response) {
...
export function createHttpAgentNativeExtensionStorage({ endpoint, fetch: fetchImpl, headers, credentials = "same-origin", }) {
    const run = async (operation, collection, context, options, id, data) => {
        const requestFetch = fetchImpl ?? fetch;
        if (typeof requestFetch !== "function") {
            throw new Error("fetch is not available for extension storage");
        }
        const resolvedHeaders = new Headers(await resolveHttpStorageHeaders(headers, context));
        ...
        return readHttpStorageResponse(response);
    };
    return {
        list(collection, options, context) {
            return run("list", collection, context, options);
        },
        get(collection, id, options, context) {
            return run("get", collection, context, options, id);
        },
        set(collection, id, data, options, context) {
            return run("set", collection, context, options, id, data);
        },
        remove(collection, id, options, context) {
            return run("remove", collection, context, options, id);
        },
    };
}
export function normalizeAgentNativeExtensionSandbox(sandbox) {
    const tokens = new Set((sandbox ?? "allow-scripts allow-forms allow-popups allow-downloads")
        .split(/\s+/)
        .filter(Boolean));
...
export function buildAgentNativeExtensionHtml({ extensionId, content, title, slotId, slotContext, themeCss = "", isDark = false, }) {
...
//# sourceMappingURL=portable-extension.js.map