...
function write(popularity) {
    if (typeof window === "undefined")
        return;
    try {
        window.localStorage.setItem(EXTENSION_POPULARITY_KEY, JSON.stringify(popularity));
    }
    catch {
...
export function incrementExtensionView(id) {
    if (!id)
        return;
...
export function useExtensionPopularity() {
...
export function extensionPopularityOf(popularity, id) {
    return popularity[id] ?? 0;
}
//# sourceMappingURL=extension-popularity.js.map