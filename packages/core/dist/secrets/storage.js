 ...
 * record's id.
 */
export async function writeAppSecret(args) {
    await ensureTable();
    const { key, value, scope, scopeId, description, urlAllowlist } = args;
    if (!key || !value || !scope || !scopeId) {
        throw new Error("writeAppSecret: key, value, scope, and scopeId are all required");
    }
    const client = getDbExec();
    const now = Date.now();
 ...
 * over the wire.
 */
export async function getAppSecretMeta(ref) {
 ...
 */
export async function readAppSecretMeta(ref) {
    await ensureTable();
    const { key, scope, scopeId } = ref;
    const client = getDbExec();
    const { rows } = await client.execute({
 ...
 * user-created keys.
 */
export async function listAppSecretsForScope(scope, scopeId) {
    await ensureTable();
    const client = getDbExec();
    const { rows } = await client.execute({
...
export async function deleteAppSecret(ref) {
    await ensureTable();
    const { key, scope, scopeId } = ref;
    const client = getDbExec();
