...
export function agentsBundlePlugin() {
    ...
    return {
        ...
        async configureServer(server) {
            ...
            let workspaceAgentsMdPath = null;
            let workspaceSkillsDir = null;
            try {
                const ws = await getWorkspaceCoreExports(projectRoot);
                if (ws) {
            ...
            const shouldInvalidate = (file) => {
                const rel = path.relative(projectRoot, file);
                if (!rel.startsWith("..")) {
                    if (rel === "AGENTS.md")
                        return true;
                    if (rel.startsWith(".agents" + path.sep + "skills") &&
...
//# sourceMappingURL=agents-bundle-plugin.js.map