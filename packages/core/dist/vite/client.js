 ...
 */
function findWorkspaceCoreSync(startDir) {
...
function getClientDedupe(cwd) {
    ...
    try {
        ...
        const appPkg = JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf-8"));
        const appDeps = new Set([
            ...Object.keys(appPkg.dependencies ?? {}),
            ...Object.keys(appPkg.devDependencies ?? {}),
        ]);
        for (const dep of coreDeps) {
            if (serverOnly.has(dep))
                continue;
...
function getDefaultOptimizeDeps(cwd) {
    ...
    return entries
        .filter(({ specifier, packageName }) => hasDep(packageName ?? specifier, cwd))
 ...
 */
function getCoreSourceAliases(cwd) {
    const coreSrc = findCoreSrcDir(cwd);
    if (!coreSrc)
        return []; // Not in monorepo — use dist as normal
    ...
    const entries = {
        ...
        "@agent-native/core/application-state": path.join(coreSrc, "application-state/index.ts"),
 ...
 */
function autoReloadOnOptimizeDep() {
    return {
 ...
 */
function fullReloadOnOptimizeDep504() {
    return {
            ...
            server.middlewares.use((req, res, next) => {
                ...
                if (base && base !== "/" && req.url?.startsWith(base)) {
                    ...
                    try {
                        ...
                        const publicPath = path.normalize(path.join(publicDir, decodeURIComponent(url.pathname)));
...
function devPathname(reqUrl) {
    return new URL(reqUrl, "http://agent-native.local").pathname;
}
function isApiDevPath(reqUrl) {
    const pathname = devPathname(reqUrl);
    return pathname === "/api" || pathname.startsWith("/api/");
}
export function stripMountedDevApiPath(reqUrl, base) {
    if (!reqUrl || !base || base === "/")
        return reqUrl;
...
export function isFrameworkDevPath(reqUrl, base) {
    const pathname = devPathname(reqUrl);
    ...
    return (pathname === `${normalizedBase}/_agent-native` ||
 ...
 */
function ssrStubPlugin(packages) {
    if (!packages.length)
        return null;
        ...
        configureServer(server) {
            ...
            if (hot?.send) {
                ...
                hot.send = (payload, ...args) => {
                    if (isBenignErrorPayload(payload))
                        return;
...
export function defineConfig(options = {}) {
    ...
    const hasReactRouterPlugin = options.plugins?.some((p) => p?.name === "react-router" ||
        (Array.isArray(p) && p.some((pp) => pp?.name === "react-router")));
    let reactTransformPlugin;
    ...
    let tailwindPluginInstance = null;
