...
function getSecret() {
    ...
    if (!secret) {
        throw new Error("A2A_SECRET is required for the integration webhook → processor handoff. " +
 ...
 */
export function signInternalToken(taskId) {
 ...
 */
export function verifyInternalToken(taskId, token) {
    if (!token)
        return false;
    ...
    if (now - ts > MAX_AGE_MS)
        return false;
    if (ts - now > FUTURE_SKEW_TOLERANCE_MS)
        return false;
    let expected;
    try {
