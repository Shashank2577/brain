    ...
    return async (nitroApp) => {
        ...
        h3.use(`${P}/process-task`, defineEventHandler(async (event) => {
            ...
            if (!process.env.A2A_SECRET) {
                if (process.env.NODE_ENV === "production") {
                    ...
                    return {
                        error: "A2A_SECRET not configured — internal token signing is required to process integration tasks in production.",
                    };
                }
        ...
        h3.use(`${P}`, defineEventHandler(async (event) => {
            ...
            if (action === "enable" && method === "POST") {
                ...
                const session = await getSession(event).catch(() => null);
                await saveIntegrationConfig(platform, { enabled: true }, "default", session?.email);
            ...
            if (action === "disable" && method === "POST") {
                ...
                if (adminCheck.ok === false)
                    return { error: adminCheck.error };
                const session = await getSession(event).catch(() => null);
                await saveIntegrationConfig(platform, { enabled: false }, "default", session?.email);
        ...
        if (process.env.DEBUG)
            console.log(`[integrations] Mounted integration routes for: ${adapters.map((a) => a.platform).join(", ")}`);
    };
}
/**
...
function getBaseUrl(event) {
    try {
        ...
        const getHeader = (name) => typeof headers.get === "function"
            ? headers.get(name)
            : headers[name];
