...
const lastCheckedTimes = new Map();
 ...
 */
export async function registerWatch(webhookUrl) {
...
async function stopWatch() {
    ...
    if (!config?.configData?.channelId)
        return;
    const accessToken = await getServiceAccountAccessToken();
    if (!accessToken)
        return;
...
async function getPageToken() {
    ...
    return config?.configData?.pageToken ?? null;
}
...
async function checkDocumentComments(fileId, accessToken, options) {
    ...
    for (const comment of comments) {
        ...
        if (!isAgentMention(comment.content, triggerKeyword))
            continue;
...
async function processComment(fileId, commentId, text, senderName, options, existingThreadId) {
    ...
    startRun(runId, capturedThreadId, async (send, signal) => {
        await runWithRequestContext({ userEmail: options.ownerEmail, orgId, isIntegrationCaller: true }, () => runAgentLoop({
            engine,
            model: options.model,
 ...
 */
export async function startGoogleDocsPoller(options) {
    if (pollerInterval) {
...
function startPollLoop(options, intervalMs) {
    ...
    if (process.env.DEBUG) {
        console.log(`[google-docs] Poll loop started (interval: ${intervalMs / 1000}s, service account: ${email ?? "not configured"})`);
    }
}
/**
 * Stop the Google Docs integration.
 */
export async function stopGoogleDocsPoller() {
    if (pollerInterval) {
...
//# sourceMappingURL=google-docs-poller.js.map