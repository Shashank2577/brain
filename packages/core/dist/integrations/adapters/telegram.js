    ...
    return {
        ...
        async verifyWebhook(event) {
            ...
            if (!secret) {
                if (shouldRefuseWhenSecretMissing()) {
                    if (!_telegramUnverifiedWarned) {
                        ...
                        console.error("[telegram] TELEGRAM_WEBHOOK_SECRET not set — refusing webhook in production. " +
            ...
            try {
                ...
                return crypto.timingSafeEqual(Buffer.from(secret), Buffer.from(headerSecret));
            }
            catch {
                return false;
            }
