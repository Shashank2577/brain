...
function splitMessage(text, maxLength) {
    if (utf8ByteLength(text) <= maxLength)
        return [text];
    const chunks = [];
    let remaining = text;
...
function markdownToSlackMrkdwn(text) {
    const bounded = text.length > MRKDWN_MAX_LENGTH ? text.slice(0, MRKDWN_MAX_LENGTH) : text;
    return (bounded
        .replace(/\[([^\]]+)\]\(([^)]+)\)/g, "<$2|$1>")
...
function buildResponseBlocks(text, opts) {
    const sectionChunks = splitMessage(text || "_(no response)_", SLACK_SECTION_TEXT_MAX_LENGTH);
    const blocks = sectionChunks.map((chunk) => ({
        type: "section",
