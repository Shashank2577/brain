...
import { runAgentLoop, actionsToEngineTools, getOwnerActiveApiKey, getOwnerApiKey, engineToProvider, } from "../agent/production-agent.js";
 ...
 */
export async function handleWebhook(event, options) {
...
async function enqueueAndDispatch(event, incoming, options) {
    ...
    await Promise.race([
        dispatchPromise,
        new Promise((resolve) => setTimeout(resolve, PROCESSOR_DISPATCH_SETTLE_WAIT_MS)),
    ]);
}
/**
    ...
    await new Promise((resolve) => {
        startRun(runId, threadId, async (send, signal) => {
            await runWithRequestContext({
                ...
                const effectiveApiKey = await resolveIntegrationApiKey(engineOption, ownerEmail, apiKey);
            ...
            try {
                ...
                const suppressPlatformReply = queuedA2AContinuation &&
                ...
                const runErrorText = completedRun.events
                    .map((runEvent) => runEvent.event.type === "error" ? runEvent.event.error : "")
                    .filter(Boolean)
                    .join("\n");
                if (isLlmCredentialError(responseText) ||
                    isLlmCredentialError(runErrorText)) {
                    responseText = formatLlmCredentialErrorMessage();
                }
                else if (!suppressPlatformReply &&
...
function extractRecoverableA2AArtifactToolResult(completedRun) {
    for (let i = completedRun.events.length - 1; i >= 0; i--) {
        ...
        if (event.type !== "tool_done" || event.tool !== "call-agent")
            continue;
        const result = String(event.result ?? "").trim();
        if (result.includes("verified artifacts already exist") &&
            result.includes("\nArtifacts:\n")) {
            return result;
        }
    }
