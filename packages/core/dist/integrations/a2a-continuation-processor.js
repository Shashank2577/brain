...
export async function dispatchA2AContinuation(continuationId, webhookBaseUrl) {
    ...
    await Promise.race([
        dispatchPromise,
        new Promise((resolve) => setTimeout(resolve, DISPATCH_SETTLE_WAIT_MS)),
    ]);
}
async function logFailedDispatchResponse(continuationId, response) {
    ...
    console.error(`[integrations] A2A continuation ${continuationId} processor dispatch returned HTTP ` +
...
async function waitForContinuationDue(continuationId) {
    ...
    if (waitMs <= 0)
        return true;
    if (waitMs > MAX_PRE_CLAIM_WAIT_MS)
        return false;
    await sleep(waitMs);
    return true;
}
async function notifyAndFailA2AContinuation(continuation, adapter, reason) {
    ...
    try {
        await withTimeout(adapter.sendResponse(adapter.formatAgentResponse(message), deliveryContinuation.incoming, { placeholderRef: deliveryContinuation.placeholderRef ?? undefined }), PLATFORM_SEND_TIMEOUT_MS, `${deliveryContinuation.platform} failure notification timed out`);
    }
    catch (err) {
...
function sanitizeFailureReason(reason) {
    ...
    return (withoutEnvNames.slice(0, 500) ||
