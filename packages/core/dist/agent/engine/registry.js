 ...
 */
export function registerAgentEngine(entry) {
...
export function getAgentEngineEntry(name) {
    return _registry.get(name);
}
/** List all registered engine entries */
export function listAgentEngines() {
...
export async function detectEngineFromUserSecrets() {
    ...
    let orgId;
    try {
        const { getRequestUserEmail, getRequestOrgId } = await import("../../server/request-context.js");
 ...
 */
export function isAgentEngineSettingConfigured(stored) {
...
export async function resolveEngine(config) {
    ...
    if (stored && typeof stored.engine === "string") {
        ...
        if (entry && (await isStoredEngineUsableForRequest(stored, entry))) {
            return entry.create({
                ...engineCreateConfig(apiKey, stripInlineApiKeyConfig(stored.config)),
            });
        }
    }
    if (detectedFromUser) {
    ...
    const detected = canUseDeployCredentialFallbackForRequest()
        ...
        : null;
    if (detected)
        return detected.create(engineCreateConfig(apiKey));
    // 9. Default: anthropic
