 ...
 */
export function engineToolsToAISDK(tools, jsonSchema) {
    const result = {};
 ...
 * `{role: "user"}` message.
 */
export function engineMessageToAISDK(msg) {
 ...
 */
export function aiSdkPartToEngineEvents(part) {
    const events = [];
    switch (part?.type) {
        case "text-delta":
            if (part.text)
 ...
 */
export function aiSdkStepToAssistantContent(step) {
    const parts = [];
...
//# sourceMappingURL=translate-ai-sdk.js.map