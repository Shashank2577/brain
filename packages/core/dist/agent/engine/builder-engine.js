...
class BuilderEngine {
    ...
    async *stream(opts) {
        ...
        const explicitReasoningEffort = normalizeReasoningEffortForModel(opts.model, opts.reasoningEffort);
        const reasoningEffort = explicitReasoningEffort ??
        ...
        try {
            ...
            try {
                ...
                if (gatewayAbort.didTimeout()) {
                    console.warn(`[builder-engine] gateway timed out after ${Date.now() - tStart}ms`);
                }
    ...
    try {
            ...
            switch (event.type) {
                ...
                case "tool-call-delta":
                    yield {
                        ...
                        id: event.id,
                        name: event.name,
                        text: typeof event.argsTextDelta === "string"
                            ? event.argsTextDelta
        ...
        if (timedOut || isBuilderGatewayNetworkError(err)) {
            captureBuilderGatewayTransportError(err, {
                ...
                timeoutMs: gatewayTimeoutMs,
                timedOut,
                elapsedMs: typeof captureContext.requestStartedAt === "number"
