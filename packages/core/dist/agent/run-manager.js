 ...
 */
export function startRun(runId, threadId, runFn, onComplete, options) {
    ...
    const bumpProgressIfDue = () => {
        const now = Date.now();
        if (now - lastProgressBumpAt < 1000)
            return;
 ...
 */
export function subscribeToRun(runId, fromSeq) {
    ...
    return new ReadableStream({
        start(controller) {
            ...
            for (let i = fromSeq; i < run.events.length; i++) {
                try {
                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ ...run.events[i].event, seq: run.events[i].seq })}\n\n`));
                }
                catch {
                    return;
                }
            ...
            subscriberRef = (event) => {
                try {
                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ ...event.event, seq: event.seq })}\n\n`));
            ...
            const poll = async () => {
                ...
                try {
                    ...
                    for (const { seq, eventData } of events) {
                        ...
                        try {
                            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ ...parsed, seq })}\n\n`));
                        }
                        catch {
                            cancelled = true;
                            return;
                    ...
                    if (events.length === 0) {
                        ...
                        if (!run || run.status !== "running") {
                            ...
                            if (run?.status === "aborted") {
                                try {
                                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: "done", seq: lastSeq })}\n\n`));
                                }
                                catch {
                                    cancelled = true;
                                    return;
...
export function getActiveRunForThread(threadId) {
 ...
 */
export async function getActiveRunForThreadAsync(threadId) {
    ...
    try {
        ...
        if (sqlRun.status === "completed" || sqlRun.status === "errored") {
            ...
            if (terminalAge > TERMINAL_RUN_RECONNECT_WINDOW_MS)
                return null;
            return {
                runId: sqlRun.id,
                threadId: sqlRun.threadId,
...
export function abortRun(runId, reason = "user") {
    const run = activeRuns.get(runId);
    if (run) {
