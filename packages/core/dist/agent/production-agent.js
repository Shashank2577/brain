...
export async function getOwnerActiveApiKey(ownerEmail) {
    try {
        ...
        const activeEngine = engineSetting?.engine ?? "anthropic";
...
export async function resolveAgentOwnerEmail(options, event) {
    let ownerEmail = null;
...
function dataUrlToFilePart(att) {
    if (att.type !== "file" || typeof att.data !== "string")
        return null;
...
function enrichMessage(message, references) {
    ...
    if (fileRefs.length > 0) {
        parts.push("Referenced files:\n" +
            fileRefs
                .map((r) => `- ${r.path}${r.source === "resource" ? " (resource)" : ""}`)
                .join("\n"));
    }
    ...
    if (customAgentRefs.length > 0) {
        parts.push("Requested custom agents:\n" +
            customAgentRefs
                .map((r) => `- ${r.name}${r.refId ? ` (id: ${r.refId})` : ""}${r.path ? ` (path: ${r.path})` : ""}`)
                .join("\n"));
    }
    if (mentionRefs.length > 0) {
        parts.push("Referenced items:\n" +
            mentionRefs
                .map((r) => `- [${r.refType || "item"}] ${r.name}${r.refId ? ` (id: ${r.refId})` : ""}${r.path ? ` (path: ${r.path})` : ""}`)
                .join("\n"));
    }
    return `${parts.join("\n\n")}\n\n${message}`;
}
 ...
 */
export function isResumableEngineError(err) {
    if (!(err instanceof Error))
        return false;
    const code = err instanceof EngineError ? (err.errorCode ?? "").toLowerCase() : "";
    if (code === "builder_gateway_timeout" ||
 ...
 * `auto_continue` event.
 */
export function continuationReasonForResumableError(err) {
    const code = err instanceof EngineError ? (err.errorCode ?? "").toLowerCase() : "";
    if (code === "builder_gateway_timeout")
...
function errorSearchText(err) {
    ...
    if (err instanceof Error) {
        ...
        if (typeof maybe.code === "string")
            parts.push(maybe.code);
        if (maybe.cause)
            parts.push(errorSearchText(maybe.cause));
    }
    else {
...
export async function runAgentLoop(opts) {
    ...
    while (true) {
        ...
        if (toolCallErrors.size > 0) {
            const existingToolCallIds = new Set(assistantContent
...
export function createProductionAgentHandler(options) {
    ...
    return defineEventHandler(async (event) => {
        ...
        if (preparedRequest) {
            if (typeof preparedRequest.message === "string" &&
            ...
            try {
                ...
                if (viewScreenAction) {
                    ...
                    if (result && result !== "(no output)") {
                        const screenText = typeof result === "string"
                            ? result
        ...
        const urlContextPromise = (async () => {
            try {
                ...
                if (url && (url.pathname || url.search || url.hash)) {
                    ...
                    if (url.search)
                        lines.push(`search: ${url.search}`);
                    if (url.hash)
                        lines.push(`hash: ${url.hash}`);
        ...
        if (systemPromptError) {
            ...
            return new ReadableStream({
                start(controller) {
                    controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: "error", error: `Failed to load system prompt: ${err?.message ?? String(err)}` })}\n\n`));
                    controller.close();
                },
            });
        }
                    ...
                    try {
                        ...
                        try {
                            for await (const task of a2aClient.stream({
                                ...
                                : undefined)) {
                                const newText = task.status?.message?.parts
...
//# sourceMappingURL=production-agent.js.map