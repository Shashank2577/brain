      ...
      `);
            ...
            try {
                if (isPostgres()) {
                    ...
                    await client.execute(`ALTER TABLE agent_runs ADD COLUMN heartbeat_at ${intType()}`);
                }
            }
            catch {
            ...
            try {
                if (!isPostgres()) {
                    await client.execute(`ALTER TABLE agent_runs ADD COLUMN abort_reason TEXT`);
                }
            }
            catch {
                // Column already exists — ignore
            ...
            try {
                if (isPostgres()) {
                    await client.execute(`ALTER TABLE agent_runs ADD COLUMN IF NOT EXISTS last_progress_at ${intType()}`);
                }
                else {
                    await client.execute(`ALTER TABLE agent_runs ADD COLUMN last_progress_at ${intType()}`);
                }
            }
            catch {
                // Column already exists — ignore
 ...
 */
export async function bumpRunProgress(runId) {
    await ensureRunTables();
    const client = getDbExec();
    await client.execute({
...
export async function getRunAbortState(runId) {
    await ensureRunTables();
    const client = getDbExec();
...
export async function getRunByThread(threadId, options) {
    await ensureRunTables();
    const client = getDbExec();
...
export async function cleanupOldRuns(olderThanMs) {
    await ensureRunTables();
    const client = getDbExec();
 ...
 */
export async function ensureTerminalRunEvent(runId, event) {
