...
import { getOrgSetting, putOrgSetting, deleteOrgSetting, getUserSetting, putUserSetting, deleteUserSetting, } from "../settings/index.js";
...
export function normalizeMaxIterations(value, fallback = DEFAULT_AGENT_MAX_ITERATIONS) {
    const parsed = parseInteger(value);
    if (parsed == null)
        return fallback;
    return Math.min(MAX_AGENT_MAX_ITERATIONS, Math.max(MIN_AGENT_MAX_ITERATIONS, parsed));
}
export function validateMaxIterationsInput(value) {
    const parsed = parseInteger(value);
    if (parsed == null) {
...
export function getDefaultMaxIterations() {
    return normalizeMaxIterations(process.env.AGENT_MAX_ITERATIONS, DEFAULT_AGENT_MAX_ITERATIONS);
}
...
export async function writeAgentLoopSettings(ctx, maxIterations) {
...
export async function canUpdateAgentLoopSettings(userEmail, orgId) {
    if (!userEmail)
        return false;
    if (!orgId)
        return true;
...
//# sourceMappingURL=loop-settings.js.map