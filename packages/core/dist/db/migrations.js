import { getDbExec, isPostgres, getDialect, retrySqliteBusy, } from "./client.js";
/**
...
export function runMigrations(migrations, options) {
    const table = options?.table;
    if (!table ||
        typeof table !== "string" ||
        !/^[A-Za-z_][A-Za-z0-9_]*$/.test(table)) {
        throw new Error("runMigrations: `table` option is required and must be a valid SQL identifier " +
    ...
    return async () => {
        try {
            ...
            if (d1) {
                await d1
                    .prepare(`CREATE TABLE IF NOT EXISTS ${table} (version INTEGER PRIMARY KEY)`)
                    .run();
                const firstRow = await d1
                    .prepare(`SELECT MAX(version) as v FROM ${table}`)
                    .first();
                const current = firstRow?.v ?? 0;
                        ...
                        if (hasIfNotExists) {
                            ...
                            for (const { sql: stmt, hadIfNotExists } of statements) {
                                try {
                                    ...
                                    if (hadIfNotExists && isDuplicateColumnError(err))
                                        continue;
                                    throw err;
                                }
                            }
            ...
            await retrySqliteBusy(() => exec.execute(`CREATE TABLE IF NOT EXISTS ${table} (version INTEGER PRIMARY KEY)`), { maxAttempts: 6, baseDelayMs: 1000, rethrow: true });
            ...
            if (pending.length > 0) {
                console.log(`[db] Applying ${pending.length} migration(s) on ${pg ? "Postgres" : "SQLite/libsql"}…`);
            }
