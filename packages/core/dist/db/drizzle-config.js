 ...
 */
function isDrizzlePushInvocation() {
    const argv = process.argv.map((a) => a.toLowerCase());
    const joined = argv.join(" ");
    if (/\bdrizzle-kit\b/.test(joined) && /\bpush\b/.test(joined))
        return true;
    ...
    const lifecycleScript = (process.env.npm_lifecycle_script ||
        ...
        "").toLowerCase();
    if (/\bdrizzle-kit\s+push\b/.test(lifecycleScript))
        return true;
    return false;
}
...
export function createDrizzleConfig(opts = {}) {
    const { schema = "./server/db/schema.ts", out = "./server/db/migrations", sqliteFile = "./data/app.db", } = opts;
    ...
    const envUrl = (appName && process.env[`${appName}_DATABASE_URL`]) ||
        process.env.DATABASE_URL ||
        "";
    const envAuthToken = (appName && process.env[`${appName}_DATABASE_AUTH_TOKEN`]) ||
    ...
    if (isTurso && !envAuthToken) {
        throw new Error("createDrizzleConfig: DATABASE_URL is a libsql:// URL but DATABASE_AUTH_TOKEN " +
