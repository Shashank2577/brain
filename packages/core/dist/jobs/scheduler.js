...
export function parseJobFrontmatter(content) {
    ...
    const meta = { schedule: "", enabled: true };
...
export function buildJobContent(meta, body) {
    ...
    if (meta.createdBy)
        lines.push(`createdBy: ${meta.createdBy}`);
    if (meta.orgId)
        lines.push(`orgId: ${meta.orgId}`);
    if (meta.runAs)
        lines.push(`runAs: ${meta.runAs}`);
    if (meta.lastRun)
        lines.push(`lastRun: ${meta.lastRun}`);
    if (meta.lastStatus)
        lines.push(`lastStatus: ${meta.lastStatus}`);
    if (meta.lastError)
        lines.push(`lastError: "${meta.lastError.replace(/"/g, '\\"')}"`);
    if (meta.nextRun)
        lines.push(`nextRun: ${meta.nextRun}`);
    lines.push(`---`);
    lines.push("");
    lines.push(body);
    return lines.join("\n");
    ...
    try {
        ...
        for (const resource of jobResources) {
            ...
            if (!resource.path.endsWith(".md"))
                continue;
            if (resource.path.endsWith(".keep"))
                continue;
            const { meta, body } = parseJobFrontmatter(resource.content);
            // Skip disabled or missing schedule
            if (!meta.enabled || !meta.schedule)
                continue;
            if (!isValidCron(meta.schedule))
                continue;
...
async function executeJob(resource, meta, body, deps, now) {
    ...
    const jobUserEmail = effectiveRunAs === "creator"
    ...
    if (!validity.ok) {
        console.warn(`[recurring-jobs] Skipping job "${jobName}": ${validity.reason}. ` +
    ...
    await updateResource(resource, meta, body);
    await runWithRequestContext({ userEmail: jobUserEmail, orgId: jobOrgId }, async () => {
        try {
