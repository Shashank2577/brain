 ...
 */
export async function resolveOrgIdForEmail(email) {
    const exec = getDbExec();
    if (!exec)
        return null;
    try {
...
async function tryCreateDefaultOrg(exec, email, session) {
    ...
    if (!(await acquireClaim(exec, claimKey)))
        return null;
 ...
 */
export async function getOrgDomain(orgId) {
    try {
        const exec = getDbExec();
        const { rows } = await exec.execute({
 ...
 */
export async function getOrgA2ASecret(orgId) {
    try {
        const exec = getDbExec();
        const { rows } = await exec.execute({
        ...
        if (!rows[0])
            return null;
        const secret = String(rows[0].a2a_secret || "");
        return secret || null;
    }
    catch {
        return null;
 ...
 */
export async function getA2ASecretByDomain(domain) {
    try {
        const exec = getDbExec();
        const { rows } = await exec.execute({
 ...
 */
export async function resolveOrgByDomain(domain) {
    try {
        const exec = getDbExec();
        const { rows } = await exec.execute({
