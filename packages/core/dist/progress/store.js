...
import { getDbExec, intType, isUniqueViolation, retryOnDdlRace, safeJsonParse, } from "../db/client.js";
...
export async function insertRun(input) {
    ...
    try {
        ...
        if (input.id && isUniqueViolation(err)) {
            throw new Error(`insertRun: run id "${input.id}" already exists for this owner`);
        }
        throw err;
    }
...
export async function getRun(id, owner) {
    await ensureTable();
    const client = getDbExec();
...
export async function updateRun(id, owner, input) {
    await ensureTable();
    const client = getDbExec();
...
export async function deleteRun(id, owner) {
    await ensureTable();
    const client = getDbExec();
    const res = await client.execute({
