 ...
 */
import { startRun, updateRunProgress, completeRun, listRuns, } from "./registry.js";
...
export function createProgressToolEntries(getCurrentUser) {
    return {
        "manage-progress": {
            tool: {
            ...
            run: async (args) => {
                ...
                switch (action) {
                    case "start": {
                        ...
                        if (!title)
                            return "Error: title is required for the start action.";
                        let metadata;
                    ...
                    case "complete": {
                        ...
                        const run = await completeRun(runId, owner, status, args.step ? { step: String(args.step) } : undefined);
                        if (!run)
                            return `Error: run ${runId} not found.`;
                    ...
                    case "list": {
                        ...
                        return rows
                            .map((r) => `[${r.status}] ${r.title}${r.percent != null ? ` · ${r.percent}%` : ""}${r.step ? ` — ${r.step}` : ""} · ${r.startedAt}`)
                            .join("\n");
                    }
                    default:
