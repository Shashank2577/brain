...
export function emit(event, payload, meta) {
    ...
    if (def) {
        ...
        if (result instanceof Promise) {
            console.warn(`[event-bus] Payload schema for "${event}" returned a Promise — ` +
        ...
        console.warn(`[event-bus] Emitting unregistered event "${event}". ` +
...
export function listSubscriptions(event) {
    const bus = getBus();
    const out = [];
    for (const [id, sub] of bus.subscriptions) {
        if (event && sub.event !== event)
            continue;
...
export function __resetEventBus() {
    const bus = getBus();
