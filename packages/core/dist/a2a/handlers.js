 ...
 */
export async function processA2ATaskFromQueue(taskId, config, event) {
    ...
    const processorMeta = (meta.__a2a_processor ?? {});
    const verifiedEmail = processorMeta.verifiedEmail;
    const orgDomainHint = processorMeta.orgDomainHint;
    const contextId = processorMeta.contextId ?? undefined;
    const callerMetadata = processorMeta.callerMetadata ?? undefined;
    ...
    heartbeat.unref?.();
    try {
        await runWithRequestContext({ userEmail: verifiedEmail, orgId: resolvedOrgId }, () => runHandlerAndPersist(taskId, message, config, contextId, callerMetadata, event));
    }
    catch (err) {
        try {
            await updateTask(taskId, {
...
async function withA2ARequestContext(metadata, event, fn) {
    ...
    const orgDomain = event?.context?.__a2aOrgDomain ?? undefined;
    const resolvedOrgId = await resolveVerifiedA2AOrgId(verifiedEmail, orgDomain);
    return runWithRequestContext({ userEmail: verifiedEmail, orgId: resolvedOrgId }, fn);
}
...
async function handleSend(params, config, event) {
    ...
    if (asyncMode) {
        ...
        const orgDomainHint = event?.context?.__a2aOrgDomain ?? undefined;
        const taskMetadata = {
            ...(metadata ?? {}),
            __a2a_processor: {
                verifiedEmail,
...
async function handleStream(params, config, res, event) {
    ...
    await withA2ARequestContext(metadata, event, async () => {
        ...
        try {
            ...
            res.write(`data: ${JSON.stringify(jsonRpcError(0, -32000, err.message ?? "Handler failed"))}\n\n`);
        }
        res.end();
    });
}
...
async function handleGet(params, event, config) {
    ...
    const taskChanged = await refireStuckAsyncTaskIfNeeded(id, event).catch((err) => {
    ...
    if (taskChanged) {
        ...
        if (updated)
            return jsonRpcResult(0, sanitizeTaskForResponse(updated));
    }
 ...
 */
export async function handleJsonRpcH3(body, event, config) {
    ...
    switch (body.method) {
        ...
        case "message/stream": {
            ...
            return undefined; // Response already sent via SSE
        }
        case "tasks/get": {
