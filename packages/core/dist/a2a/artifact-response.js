...
function imageIdValue(parsed) {
    return (stringValue(parsed.assetId) ??
...
function addImageArtifact(images, parsed) {
    ...
    images.set(id, {
        ...
        url: stringValue(parsed.pageUrl) ??
...
function addDeckArtifact(decks, parsed, options) {
    ...
    if (!id)
        return;
    if (options.requireReady && !isReadyDeckArtifact(parsed))
        return;
    decks.set(id, {
        id,
...
function collectArtifacts(results) {
    ...
    for (const toolResult of results) {
        ...
        if (toolResult.tool === "update-dashboard" ||
        ...
        if (toolResult.tool === "save-analysis" ||
...
function parseDownstreamArtifactBlock(result) {
    ...
    for (const line of downstreamArtifactLines(result)) {
        ...
        if (deck) {
            const id = deck[2];
            if (!artifactUrlReferencesId(deck[1], "deck", id))
                continue;
            artifacts.push({
                kind: "deck",
                url: deck[1],
        ...
        if (document) {
            const id = document[3];
            if (!artifactUrlReferencesId(document[2], "document", id))
                continue;
            artifacts.push({
                kind: "document",
                title: document[1],
        ...
        if (dashboard) {
            ...
            if (!artifactUrlReferencesId(dashboard[2], "dashboard", id))
                continue;
            artifacts.push({
                kind: "dashboard",
                title: dashboard[1],
        ...
        if (analysis) {
            const id = analysis[3];
            if (!artifactUrlReferencesId(analysis[2], "analysis", id))
                continue;
            artifacts.push({
                kind: "analysis",
                title: analysis[1],
        ...
        if (image) {
            const id = image[3];
            if (!artifactUrlReferencesId(image[2], "image", id))
                continue;
            artifacts.push({
                kind: "image",
                title: image[1],
        ...
        if (design) {
            const id = design[2];
            if (!artifactUrlReferencesId(design[1], "design", id))
                continue;
            artifacts.push({
                kind: "design",
                url: design[1],
...
function collectReferencedArtifacts(text, baseUrl) {
    ...
    const artifactUrlPattern = /(?:(https?:\/\/[^/\s<>()]+))?(?:\/[^\s<>()]*)?\/(deck|design|page|adhoc|analyses|image|asset|assets)\/([A-Za-z0-9_-]+)/g;
    for (const match of text.matchAll(artifactUrlPattern)) {
        const origin = safeOrigin(match[1]);
        const route = match[2];
        const id = match[3];
        const kind = route === "deck"
            ? "deck"
            : route === "design"
                ? "design"
                : route === "page"
                    ...
                    : route === "adhoc"
                        ...
                        : route === "analyses"
                            ? "analysis"
                            : "image";
        if (!shouldValidateArtifactReference(origin, baseOrigin, kind))
            continue;
...
const KNOWN_AGENT_NATIVE_ARTIFACT_HOSTS = {
...
function findUnverifiedArtifactReferences(text, baseUrl, documents, decks, dashboards, analyses, images, generatedDesigns) {
    ...
    return collectReferencedArtifacts(text, baseUrl).filter((ref) => {
        ...
        if (ref.kind === "dashboard")
            return !dashboardIds.has(ref.id);
        if (ref.kind === "analysis")
            return !analysisIds.has(ref.id);
        if (ref.kind === "image")
            return !imageIds.has(ref.id);
