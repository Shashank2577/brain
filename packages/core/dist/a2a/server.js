 ...
 */
export function mountA2A(nitroApp, config, routePrefix = "/_agent-native") {
    ...
    getH3App(nitroApp).use("/.well-known/agent-card.json", defineEventHandler((event) => {
        ...
        const protocol = getRequestHeader(event, "x-forwarded-proto") ||
...
export function filterPublicAgentCardSkills(config) {
    return (config.skills ?? []).filter((skill) => {
        ...
        if (!config.publicSkillsOnly)
            return true;
        if (skill.publicAgent) {
            return (skill.publicAgent.expose === true &&
