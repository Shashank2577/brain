 ...
 */
export async function claimA2ATaskForProcessing(id) {
    await ensureTable();
    const client = getDbExec();
    const now = Date.now();
    const timestamp = new Date().toISOString();
...
export async function getA2ATaskDispatchState(id) {
    await ensureTable();
    const client = getDbExec();
    const { rows } = await client.execute({
...
export async function touchProcessingA2ATask(id) {
    await ensureTable();
    const client = getDbExec();
    const now = Date.now();
    const result = await client.execute({
...
export async function resetStuckA2ATaskForRetry(id, processingCutoff) {
    await ensureTable();
    const client = getDbExec();
    const now = Date.now();
    const timestamp = new Date().toISOString();
...
export async function failStuckA2ATask(id, processingCutoff, reason) {
    await ensureTable();
    const client = getDbExec();
    const now = Date.now();
    const timestamp = new Date().toISOString();
...
export async function listTasks(contextId) {
    ...
    const { rows } = await client.execute(`SELECT * FROM a2a_tasks ORDER BY created_at DESC`);
    return rows.map(taskFromRow);
}
