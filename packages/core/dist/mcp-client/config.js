...
function parseConfig(raw, source) {
    try {
        ...
        if (!parsed || typeof parsed !== "object")
            return null;
        const servers = parsed.servers && typeof parsed.servers === "object"
            ? parsed.servers
            : null;
        if (!servers)
            return null;
 ...
 */
export function loadMcpConfig(startDir) {
...
export function autoDetectMcpConfig() {
    if (!isNode())
        return null;
    if (process.env.AGENT_NATIVE_DISABLE_MCP_AUTODETECT)
        return null;
    const candidates = [];
    const home = os.homedir();
    if (home) {
        candidates.push(path.join(home, ".claude-in-chrome", "bin", "claude-in-chrome-mcp"));
    }
