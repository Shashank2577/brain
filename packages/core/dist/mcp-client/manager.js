...
export function parseMcpToolName(prefixedName) {
    if (!prefixedName.startsWith(MCP_TOOL_PREFIX))
        return null;
...
export class McpClientManager {
    ...
    async addServer(id, cfg, sdk) {
        if (this.servers.has(id)) {
            console.warn(`[mcp-client] Duplicate server ID '${id}' — overwriting previous registration`);
        }
        const entry = {
            id,
            config: cfg,
    ...
    async connectServer(entry, sdk) {
        ...
        if (cfg.type === "http") {
            ...
            if (!sdk.StdioClientTransport) {
                throw new Error("Stdio transport not available (needs Node runtime with MCP SDK)");
            }
    ...
    async reconfigureInternal(newConfig) {
        ...
        if (toConnect.length > 0) {
            ...
            const sdk = await this.loadSdk(needStdio);
            if (sdk) {
                await Promise.all(toConnect.map((id) => this.addServer(id, nextServers[id], sdk)));
            }
        }
    ...
    async callTool(prefixedName, args) {
        ...
        if (!entry || !entry.client) {
            throw new Error(`MCP server "${parsed.serverId}" is not connected${entry?.error ? `: ${entry.error}` : ""}`);
        }
        ...
        if (!known) {
            throw new Error(`MCP server "${parsed.serverId}" does not expose tool "${parsed.toolName}"`);
        }
