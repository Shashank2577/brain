import { resourceGet, resourceGetByPath, resourceListAccessible, SHARED_OWNER, } from "./store.js";
import { parseCustomAgentProfile, } from "./metadata.js";
export async function listAccessibleCustomAgents(owner) {
    const resources = await resourceListAccessible(owner, "agents/");
    const profiles = await Promise.all(resources
...
export async function findAccessibleCustomAgent(owner, identifier) {
    ...
    return (agents.find((agent) => agent.id.toLowerCase() === lower ||
        agent.name.toLowerCase() === lower ||
        agent.path.toLowerCase() === lower) ?? null);
}
