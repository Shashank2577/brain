...
export function createTerminalPlugin(options = {}) {
    return async (nitroApp) => {
        ...
        getH3App(nitroApp).use("/_agent-native/available-clis", defineEventHandler(async () => {
            try {
                const { CLI_REGISTRY, commandExists } = await import("./cli-registry.js");
                const results = [];
        ...
        try {
            ...
            if (missingPty) {
                if (!_ptyMissingLogged &&
                    ...
                    console.log("[terminal] node-pty not installed — embedded terminal disabled. " +
                ...
                console.error("[terminal] If node-pty is installed but PTY fails to spawn, " +
