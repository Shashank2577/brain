...
const llmStep = {
    ...
    isComplete: async () => {
        try {
            const { resolveHasBuilderPrivateKey } = await import("../server/credential-provider.js");
        ...
        try {
            if (await detectEngineFromUserSecrets())
                return true;
        }
        catch {
            // Fall through to legacy/env detection.
        }
        if (PROVIDER_ENV_VARS.some((k) => !!process.env[k]))
            return true;
        try {
...
export function registerDefaultOnboardingSteps() {
    if (registered)
        return;
    registered = true;
...
//# sourceMappingURL=default-steps.js.map