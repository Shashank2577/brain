...
export function createOnboardingPlugin(options = {}) {
    return async (nitroApp) => {
        ...
        getH3App(nitroApp).use(`${ONBOARDING_PREFIX}/dismiss`, defineEventHandler(async (event) => {
            ...
            const { sessionId } = await resolveOnboardingContext(event);
            await appStatePut(sessionId, DISMISSED_KEY, { dismissed: true, at: new Date().toISOString() }, { requestSource: "agent" });
            return { ok: true };
        }));
        ...
        getH3App(nitroApp).use(`${ONBOARDING_PREFIX}/reopen`, defineEventHandler(async (event) => {
            ...
            const { sessionId } = await resolveOnboardingContext(event);
            await appStatePut(sessionId, DISMISSED_KEY, { dismissed: false, at: new Date().toISOString() }, { requestSource: "agent" });
            return { ok: true };
        }));
