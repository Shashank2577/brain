 ...
 */
export function registerOnboardingStep(step) {
    ...
    if (steps.has(step.id)) {
        ...
        if (process.env.DEBUG) {
            console.log(`[agent-native] Overriding onboarding step "${step.id}" with new registration.`);
        }
    }
...
export function __resetOnboardingRegistry() {
    steps.clear();
}
