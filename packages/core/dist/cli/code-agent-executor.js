...
export async function executeCodeAgentRun(options) {
    ...
    const engine = options.engine ?? (await resolveExecutorEngine(requestedEngine));
    if (!engine) {
        const message = "No LLM provider key was found. Set ANTHROPIC_API_KEY, OPENAI_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY, or another supported provider key and resume this run.";
    ...
    try {
        ...
        if (pendingFollowUp) {
            const message = pendingFollowUp.mode === "queued"
...
async function startCodeAgentMcpManager(runId) {
...
function runWithOptionalCodeAgentRequestContext(run, fn) {
    const userEmail = metadataString(run, "ownerEmail") ??
...
function hasAnyProviderCredential() {
    ...
    if (PROVIDER_ENV_VARS.some((key) => Boolean(process.env[key])))
        return true;
    return Boolean(process.env.BUILDER_PRIVATE_KEY && process.env.BUILDER_PUBLIC_KEY);
}
function createFakeCodeAgentEngine(text) {
    return {
...
export function classifyCodeAgentCommandPermission(command) {
...
function permissionErrorForWrite(permissionMode, toolName) {
...
function requestCodeAgentApproval(runId, input) {
...
function getPendingApproval(runId) {
    ...
    return {
        ...
        permissionMode: candidate.permissionMode === "read-only" ||
