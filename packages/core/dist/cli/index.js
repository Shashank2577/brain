...
function buildRedactedCommandTag(argv) {
    const out = [];
...
const FEEDBACK_URL = "https://forms.agent-native.com/f/agent-native-feedback/_16ewV?source=cli";
...
function handleScaffoldImportError(err) {
    const msg = err?.message ?? String(err);
    const looksLikeCorruptCache = err?.code === "ERR_MODULE_NOT_FOUND" ||
        err?.code === "MODULE_NOT_FOUND" ||
        err?.code === "ENOENT" ||
        /Cannot find module|tarball|integrity|EINTEGRITY|corrupt|truncated/i.test(msg);
...
function isReactRouterFramework() {
    return (fs.existsSync(path.resolve("react-router.config.ts")) ||
...
function isWorkspaceRoot() {
    ...
    try {
        ...
        return (typeof pkg?.["agent-native"]?.workspaceCore === "string" &&
 ...
 */
function inferBuildContext(cwd) {
 ...
 */
function runBuildStep(cmd, cmdArgs, opts) {
    return new Promise((resolve) => {
        ...
        child.on("exit", (code, signal) => {
            ...
            const err = new Error(`Build step "${opts.label}" failed with exit code ${exitCode}` +
...
switch (command) {
        ...
        (async () => {
            ...
            if (isReactRouterFramework()) {
                ...
                if (fs.existsSync(deployBuild)) {
                    ...
                    console.warn(`[build] Deploy build script not found at ${deployBuild}. Skipping post-build step.`);
                }
            }
            console.log("\nBuild complete.");
        })().catch((err) => {
    ...
    case "start": {
        ...
        if (!fs.existsSync(serverEntry)) {
            console.error('No production build found. Run "agent-native build" first.');
            process.exit(1);
        }
    ...
    case "create": {
        ...
        import("./create.js")
            .then((m) => m.createApp(parsed.name, {
    ...
    case "create-workspace": {
        ...
        import("./create-workspace.js")
            .then((m) => m.createWorkspace({ name: parsed.name, template: parsed.template }))
            .catch(handleScaffoldImportError);
        break;
    }
    case "add-app": {
        ...
        const parsed = parseScaffoldArgs(args);
        import("./create.js")
            .then((m) => m.addAppToWorkspace(parsed.name, { template: parsed.template }))
            .catch(handleScaffoldImportError);
        break;
    }
    case "deploy": {
