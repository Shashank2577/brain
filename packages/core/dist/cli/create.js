...
async function createWorkspaceInteractive(name, opts, clack) {
    ...
    try {
        ...
        s.stop(`Workspace scaffolded with ${templates.length} app${templates.length === 1 ? "" : "s"}.`);
    }
    catch (err) {
    ...
    if (Object.keys(catalog).length > 0) {
        ...
        if (!existing.includes("catalog:")) {
            const catalogYaml = Object.entries(catalog)
                .map(([k, v]) => `  "${k}": "${v}"`)
                .join("\n");
            fs.writeFileSync(wsPath, existing.trimEnd() + "\ncatalog:\n" + catalogYaml + "\n");
        }
    }
 ...
 */
function cleanupOnFailure(targetDir) {
    try {
        if (fs.existsSync(targetDir)) {
 ...
 * templates need it.
 */
async function scaffoldRequiredPackages(templateNames, workspaceRoot) {
    const needed = new Set();
    for (const t of templateNames) {
        const meta = getTemplate(t);
        if (meta?.requiredPackages) {
            for (const p of meta.requiredPackages)
                needed.add(p);
        }
    }
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            const existing = Array.isArray(pkg.pnpm.onlyBuiltDependencies)
                ...
                : [];
            pkg.pnpm.onlyBuiltDependencies = Array.from(new Set([...existing, ...requiredBuilt]));
...
async function promptTemplatePicker(preselected, clack, opts) {
    ...
    const options = orderedTemplates
        ...
        label: recommendedNames.has(t.name)
            ? `${t.label} (recommended)`
            : t.label,
        hint: recommendedNames.has(t.name) && t.name === "dispatch"
...
function tarExtractArgs(tarPath, destDir, options = {}) {
...
async function downloadGitHubSubdir(repo, subdir, targetDir) {
    ...
    for (const ref of refs) {
        ...
        try {
            ...
            errors.push(`  ${ref}: ${err instanceof Error ? err.message.split("\n")[0] : String(err)}`);
        }
        finally {
...
function fixPackageJsonName(appDir, name, templateName) {
...
function getCoreDependencyVersion() {
    if (process.env.AGENT_NATIVE_CREATE_USE_LOCAL_CORE === "1") {
        ...
        if (localCore)
            return pathToFileURL(localCore).href;
    }
 ...
 */
function getGitHubTemplateRefCandidates() {
...
function getGitHubTemplateRef() {
    return getGitHubTemplateRefCandidates()[0];
}
function rewriteCoreDependencyVersions(projectDir) {
...
function upsertTomlBuildEnvironment(content, vars) {
    const lines = content.split("\n");
    const sectionIndex = lines.findIndex((line) => line.trim() === "[build.environment]");
    if (sectionIndex === -1) {
        const envLines = ["", "[build.environment]"].concat(Object.entries(vars).map(([key, value]) => `  ${key} = "${value}"`));
        return content.trimEnd() + "\n" + envLines.join("\n") + "\n";
    }
    let nextSectionIndex = lines.findIndex((line, index) => index > sectionIndex && /^\s*\[/.test(line));
    if (nextSectionIndex === -1)
        nextSectionIndex = lines.length;
    for (const [key, value] of Object.entries(vars)) {
        const existingIndex = lines.findIndex((line, index) => index > sectionIndex &&
...
function ensureRedirect(content, from, to, status) {
    const escapedFrom = from.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const redirectPattern = new RegExp(`\\n?\\[\\[redirects\\]\\]\\s+from\\s*=\\s*"${escapedFrom}"\\s+to\\s*=\\s*"[^"]*"\\s+status\\s*=\\s*\\d+(?:\\s+force\\s*=\\s*(?:true|false))?`, "m");
    const block = [
        "",
        "[[redirects]]",
        `  from = "${from}"`,
...
function rewriteNetlifyToml(appDir, appName, mode) {
    ...
    try {
        ...
        const databaseSetup = 'export DATABASE_URL=\\"${NETLIFY_DATABASE_URL:-$DATABASE_URL}\\"';
        ...
        const functionsPath = mode === "workspace"
...
function rewriteTrackingAppId(appDir, appName, templateName) {
    ...
    try {
        ...
        const pattern = /(^\s*app:\s*)(["'])(?:agent-native-[^"']+|\{\{APP_NAME\}\})\2(\s*,?)/m;
...
function hasTrackingTemplate(content) {
    const match = content.match(/configureTracking\(\{[\s\S]*?\}\);/);
    return !!match && /^\s*template\s*:/m.test(match[0]);
}
function tryGitInit(dir) {
    try {
        execFileSync("git", ["init"], { cwd: dir, stdio: "pipe" });
        execFileSync("git", ["add", "-A"], { cwd: dir, stdio: "pipe" });
        execFileSync("git", ["commit", "-m", "Initial commit from agent-native create"], {
            cwd: dir,
            stdio: "pipe",
            env: {
                ...process.env,
...
function copyDir(src, dest, root) {
    ...
    for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
        ...
        if (shouldSkipScaffoldEntry(entry.name, srcPath))
            continue;
...
function shouldSkipScaffoldEntry(name, srcPath) {
