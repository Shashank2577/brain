...
export function readProjectCodePack(cwd = process.cwd(), options = {}) {
    return {
        schemaVersion: 1,
        root: cwd,
...
export function listProjectSlashCommands(cwd = process.cwd()) {
...
export function listVisibleProjectSlashCommands(cwd = process.cwd()) {
    return listProjectSlashCommands(cwd).filter((command) => !command.reserved);
}
export function findProjectSlashCommand(commandName, cwd = process.cwd()) {
    const normalized = normalizeProjectSlashCommandName(commandName);
    return (listProjectSlashCommands(cwd).find((command) => command.name === normalized) ?? null);
}
...
export function isReservedProjectSlashCommandName(value) {
    return RESERVED_PROJECT_COMMAND_NAMES.has(normalizeProjectSlashCommandName(value));
}
export function renderProjectSlashCommandPrompt(command, args) {
    const argumentText = args.join(" ").trim();
    const positional = args
        .map((arg, index) => [`$${index + 1}`, arg])
        .reduce((body, [token, value]) => body.replaceAll(token, value), command.body);
...
export function normalizeProjectSlashCommandName(value) {
    return value
        .replace(/^\//, "")
        .replaceAll("\\", "/")
        .replaceAll("/", ":")
...
function readProjectSlashCommand(root, filePath) {
    try {
        const raw = fs.readFileSync(filePath, "utf-8");
        const parsed = parseFrontmatter(raw);
        const relative = path.relative(root, filePath).replace(/\.md$/i, "");
        if (relative.toLowerCase() === "readme")
            return null;
...
function readProjectSkill(root, filePath) {
    try {
...
//# sourceMappingURL=code-agent-commands.js.map