...
export function registerNotificationChannel(channel) {
    if (!channel?.name) {
        throw new Error("registerNotificationChannel: channel.name is required");
    }
    if (typeof channel.deliver !== "function") {
        throw new Error("registerNotificationChannel: channel.deliver must be a function");
    }
...
export async function notify(input, meta) {
    ...
    results.forEach((r, i) => {
        if (r.status === "fulfilled") {
            ...
            console.error(`[notifications] channel "${channels[i].name}" failed:`, r.reason);
        }
    });
    ...
    if (delivered.length > 0) {
        try {
            emitBusEvent("notification.sent", {
...
export function __resetNotificationChannels() {
    getRegistry().clear();
}
